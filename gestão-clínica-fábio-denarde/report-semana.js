    ...
    for (const configDoc of settingsConfigSnapshot.docs) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const plan = getWhatsappReminderPlan({
                runDateStr: dateStr, // We simulate as if today is dateStr and checking AMANHA reminders
                tipo: 'AMANHA',
                patients,
            ...
            if (plan.reminders.length === 0 && plan.diagnostics.length === 0) {
                ...
                plan.reminders.forEach(r => {
                    ...
                    console.log(`  ✅ ${label} ${r.time} | ${r.patientName} (Resp: ${r.guardianName.trim()}) - Lembrete Ativo`);
                });
