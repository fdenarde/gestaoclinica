...
import { createPsychologyBackupZip, createSyntheticPsychologyStore, verifyPsychologyBackupFiles, verifyPsychologyBackupZip, type PsychologyBackupJsonResult } from './backup';
...
export default function PsychologyImportExport({ store, onGenerateBackup }: { store: PsychologyStore; onGenerateBackup?: PsychologyBackupGenerator }) {
  ...
  const handleBackup = async () => {
    ...
    try {
      if (onGenerateBackup) {
        ...
        downloadBytes(new TextEncoder().encode(result.json), result.fileName, 'application/json;charset=utf-8');
        setStatus(`Backup ${result.source === 'psychology-remote' ? 'remoto' : 'local'} gerado e baixado. Nenhum dado foi alterado.`);
      } else {
  ...
  return <section className="space-y-5" data-testid="psychology-import-export">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <section className="flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><FileOutput className="text-emerald-700" size={22} /><div><h4 className="font-black">Exportar / Fazer backup</h4><p className="mt-1 text-sm text-slate-500">{onGenerateBackup ? 'Gera JSON estruturado do escopo ativo, com versão e manifesto.' : 'Gera ZIP sintético local com manifesto e SHA-256.'}</p></div></div><button type="button" onClick={handleBackup} disabled={busy} className={`${buttonSecondary} mt-5 w-full`}><Download size={16} /> Gerar backup</button></section>
