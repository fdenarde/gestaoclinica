...
import DoctoraliaFirstPartyPreviewPanel from '../psychology-import-export/DoctoraliaFirstPartyPreviewPanel';
...
export default function PsychologyPilot({ runtimeMode }: { runtimeMode: PsychologyRuntimeMode }) {
  ...
  const loadRemoteProvider = useCallback((status: 'BOOTSTRAPPING' | 'LOADING' = 'LOADING') => {
    ...
    return remoteClient.load()
      .then(next => {
        ...
        setRemoteStore(next);
        setRemoteCapabilities(remoteClient.getCapabilities());
  ...
  const togglePatient = (patient: PsychologyPatient): boolean | Promise<boolean> => {
    if (remoteConfiguration.enabled) {
      ...
      return remoteClient.updatePatient({ ...patient, active: !patient.active })
        .then(saved => {
    ...
    <div className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900" data-testid="psychology-pilot" data-remote-provider-status={remoteConfiguration.enabled ? remoteStatus : undefined}>
        ...
        <main className="min-w-0 flex-1">
          ...
          {notice && <div className="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-800" role="status">{notice}</div>}
           ...
           {page === 'day' && <DayView date={selectedDate} setDate={setSelectedDate} store={store} sessions={daySessions} settings={store.settings} onSchedule={() => openNewEvent()} onPersonal={() => openNewEvent(selectedDate, '09:00', 'personal')} onOpenFinance={(patientId) => { const patient = patientId ? store.patients.find(item => item.id === patientId && item.active === true) : undefined; if (!patient) { openPage('finance'); return; } setPatientChartInitialTab('finance'); setPatientChart(patient); }} onOpenSession={setSessionActions} />}
             {page === 'patients' && remoteConfiguration.enabled && remoteStatus !== 'READY' ? <RemoteProviderState status={remoteStatus} onRetry={retryRemoteProvider} /> : page === 'patients' && <PatientsView rows={visiblePatients} search={search} searchKey={normalizedPatientSearch} setSearch={setSearch} onNew={() => { if (isPreview) { setNotice('Edição desabilitada nesta prévia.'); return; } if (!remoteCan('patients', 'create')) { setNotice('Criação de paciente não autorizada pelo contrato de capabilities.'); return; } setPatientDialog('new'); }} onEdit={(patient) => { if (isPreview) { setNotice('Edição desabilitada nesta prévia.'); return; } if (!remoteCan('patients', 'edit')) { setNotice('Edição de paciente não autorizada pelo contrato de capabilities.'); return; } setPatientDialog(patient); }} onOpen={patient => { setPatientChartInitialTab('summary'); setPatientChart(patient); }} onOpenFinance={patient => { setPatientChartInitialTab('finance'); setPatientChart(patient); }} onDelete={requestPatientDelete} onToggle={togglePatient} onSetReview={updatePatientReview} onBulkDelete={processBulkPatientDeletion} preview={doctoraliaPreview} />}
      ...
      {mobileMoreOpen && <div className="psychology-more-sheet-backdrop md:hidden" role="presentation" onClick={() => setMobileMoreOpen(false)}>
        <section className="psychology-more-sheet" role="dialog" aria-modal="true" aria-label="Mais opções da Psicologia" onClick={event => event.stopPropagation()}>
          ...
          {patientChart && <PsychologyPatientChart store={store} patientId={patientChart.id} initialTab={patientChartInitialTab} previewDetails={doctoraliaPreview?.patientDetailsById.get(patientChart.id)} previewClinicalBackground={doctoraliaPreview?.bundle.clinicalBackgrounds.find(item => item.externalPatientId === doctoraliaPreview.patientDetailsById.get(patientChart.id)?.externalPatientId)} readOnly={isPreview} remoteMode={remoteConfiguration.enabled} onClose={() => setPatientChart(null)} onDelete={requestPatientDelete} onEdit={(patient) => { if (isPreview) { setNotice('Edição desabilitada nesta prévia.'); return; } if (!remoteCan('patients', 'edit')) { setNotice('Edição de paciente não autorizada pelo contrato de capabilities.'); return; } setPatientChart(null); setPatientDialog(patient); }} onSchedule={(patient) => { if (isPreview) { setNotice('Novas sessões estão desabilitadas nesta prévia.'); return; } openNewEvent(today(), '09:00', 'session'); }} onOpenSession={setSessionActions} onStoreChange={updateStore} onRemoteMutation={remoteConfiguration.enabled ? persistRemoteFinanceMutation : undefined} remoteWriteBlocked={remoteConfiguration.enabled && (remoteLoading || Boolean(remoteError) || !remoteStore)} onNotice={setNotice} onStatus={(sessionId, status) => { if (remoteConfiguration.enabled) { setNotice('Provider remoto: alteração de sessão indisponível nesta etapa.'); return false; } return updateStore(updatePsychologySessionStatus(store, sessionId, status)); }} onUpdateFollowUpStatus={updatePatientFollowUpStatus} onRecord={(session) => { if (remoteConfiguration.enabled) { setNotice('Registros clínicos remotos não são carregados nem gravados nesta etapa.'); return; } setRecordDialog(session); }} />}
...
const PATIENT_LIST_GRID = 'grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1 md:grid-cols-[auto_minmax(190px,1.55fr)_minmax(120px,.8fr)_minmax(170px,1.2fr)_minmax(112px,.78fr)_max-content] md:gap-x-3 md:gap-y-0';
const PATIENT_LIST_PADDING = 'px-3 sm:px-4';
...
function PatientsView({ rows, search, searchKey, setSearch, onNew, onEdit, onOpen, onOpenFinance, onDelete, onToggle, onSetReview, onBulkDelete, preview }: { rows: PsychologyPatientListItem[]; search: string; searchKey: string; setSearch: (value: string) => void; onNew: () => void; onEdit: (patient: PsychologyPatient) => void; onOpen: (patient: PsychologyPatient) => void; onOpenFinance: (patient: PsychologyPatient) => void; onDelete: (patient: PsychologyPatient) => void; onToggle: (patient: PsychologyPatient) => boolean | Promise<boolean>; onSetReview: (patientIds: string[], inReview: boolean) => boolean | Promise<boolean>; onBulkDelete: (patientIds: string[]) => PsychologyBulkDeletionSummary | Promise<PsychologyBulkDeletionSummary>; preview?: PsychologyDoctoraliaPreview | null }) {
  ...
  const baseRows = useMemo(() => filterPsychologyPatientList(rowsWithPreviewReview, { query: searchKey, lastSession: lastSessionFilter, nextSession: nextSessionFilter, review: reviewFilter, followUpStatus: followUpStatusFilter }, referenceDate), [followUpStatusFilter, lastSessionFilter, nextSessionFilter, referenceDate, reviewFilter, rowsWithPreviewReview, searchKey]);
  const counts = useMemo(() => countPsychologyPatientList(baseRows), [baseRows]);
  const filteredRows = useMemo(() => sortPsychologyPatientList(filterPsychologyPatientList(rowsWithPreviewReview, { query: searchKey, status: statusFilter, lastSession: lastSessionFilter, nextSession: nextSessionFilter, review: reviewFilter, followUpStatus: followUpStatusFilter }, referenceDate), sortKey, sortDirection), [followUpStatusFilter, lastSessionFilter, nextSessionFilter, referenceDate, reviewFilter, rowsWithPreviewReview, searchKey, sortDirection, sortKey, statusFilter]);
  ...
  return <div data-testid="psychology-patients" className="w-full">
    ...
    <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm" data-testid="psychology-patient-filters">
      <div className="grid gap-2.5 lg:grid-cols-[minmax(220px,1.5fr)_repeat(4,minmax(135px,1fr))]">
        <label className="relative block"><span className="sr-only">Buscar paciente por nome, telefone ou e-mail</span><Search size={17} className="absolute left-3 top-3.5 text-slate-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Buscar paciente por nome, telefone ou e-mail" className={`${inputClass} pl-10`} /></label>
      ...
      {hasListAdjustments && <div className="mt-2 flex justify-end"><button type="button" onClick={clearFilters} data-testid="psychology-patient-clear-filters" className="rounded-lg px-2 py-1 text-xs font-black text-violet-700 hover:bg-violet-50">Limpar filtros</button></div>}
    </div>
    ...
    {filteredRows.length === 0 ? <EmptyState title={hasFilters ? 'Nenhum paciente encontrado' : 'Nenhum paciente cadastrado'} text={hasFilters ? 'Ajuste a busca ou os filtros para ver outros pacientes.' : 'Cadastre somente as informações essenciais para começar.'} /> : <div data-testid="psychology-patient-list" className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div data-testid="psychology-patient-list-header" role="row" className={`hidden md:grid ${PATIENT_LIST_GRID} ${PATIENT_LIST_PADDING} border-b border-slate-200 py-3 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400`}>
        <label className="flex items-center justify-center" aria-label="Selecionar todos os pacientes visíveis"><input ref={selectAllRef} type="checkbox" checked={allVisibleSelected} onChange={toggleAllVisible} disabled={Boolean(preview)} className="h-4 w-4 rounded border-slate-300 text-violet-700" /></label><SortHeader label="Paciente" sortKey="name" activeKey={sortKey} direction={sortDirection} onSort={sortBy} /><span>Telefone</span><span>Modalidade / local</span><SortHeader label="Status" sortKey="status" activeKey={sortKey} direction={sortDirection} onSort={sortBy} className="justify-center" /><span className="text-right">Ação</span>
      </div>
      <div>{filteredRows.map(row => { const rowInReview = isPsychologyPatientInReview(row); return <div key={row.patient.id} data-testid="psychology-patient-list-row" role="row" className={`group grid ${PATIENT_LIST_GRID} ${PATIENT_LIST_PADDING} border-b border-slate-100 py-2.5 transition-colors last:border-b-0 hover:bg-slate-50/70 ${rowInReview ? 'bg-amber-50/30' : ''}`}>
        ...
        <button type="button" onClick={() => onOpen(row.patient)} aria-label={`Abrir ficha completa de ${row.patient.name}`} title={row.patient.name} className="col-start-2 row-start-1 min-w-0 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-violet-500 md:col-auto md:row-auto md:truncate"><span className="block truncate text-sm font-black text-slate-900 md:text-[15px]">{row.patient.name}</span></button>
        ...
        <div data-testid="psychology-patient-list-modality" className="col-start-2 row-start-3 min-w-0 text-left text-sm text-slate-600 md:col-auto md:row-auto"><span className="mr-2 text-[10px] font-black uppercase tracking-wide text-slate-400 md:hidden">Modalidade / local</span><span className="block max-w-full truncate" title={row.modalityLocation}>{row.modalityLocation}</span></div>
        <div data-testid="psychology-patient-list-status" className="col-start-3 row-start-1 row-span-3 flex min-w-0 flex-col items-end justify-start gap-1 self-start text-right md:col-auto md:row-auto md:items-center md:justify-center md:text-center"><span className={`inline-flex rounded-full px-2 py-1 text-[11px] font-black ${row.patient.active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{row.patient.active ? 'Ativo' : 'Inativo'}</span><span className="inline-flex max-w-full rounded-full bg-sky-50 px-2 py-1 text-[10px] font-black text-sky-700">{row.patient.acompanhamentoStatus === 'PAUSADO' ? 'Pausado' : row.patient.acompanhamentoStatus === 'AGUARDANDO_RETORNO' ? 'Aguardando retorno' : row.patient.acompanhamentoStatus === 'ENCERRADO' ? 'Encerrado' : 'Acompanhamento ativo'}</span>{rowInReview && <span className="inline-flex rounded-full bg-amber-50 px-2 py-1 text-[10px] font-black text-amber-700">Em revisão</span>}</div>
        <div data-testid="psychology-patient-list-actions" className="col-span-3 row-start-4 flex min-w-0 items-center justify-end gap-1 whitespace-nowrap pt-1 text-right md:col-span-1 md:row-auto md:pt-0">
          <button type="button" onClick={() => onOpen(row.patient)} className="inline-flex items-center gap-1 whitespace-nowrap rounded-lg px-2 py-1.5 text-xs font-black text-violet-700 hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"><FileText size={14} /> Abrir ficha</button>
          <details className="relative" data-testid="psychology-patient-secondary-actions">
            <summary aria-label={`Mais ações para ${row.patient.name}`} title="Mais ações" className="flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"><MoreHorizontal size={17} /></summary>
            <div className="absolute right-0 top-full z-30 mt-1 w-44 rounded-xl border border-slate-200 bg-white p-1.5 text-left shadow-xl">
              <button type="button" onClick={() => onOpenFinance(row.patient)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-black text-emerald-700 hover:bg-emerald-50"><WalletCards size={14} /> Financeiro</button>
              {!preview && <><button type="button" onClick={() => onToggle(row.patient)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-black text-slate-600 hover:bg-slate-100">{row.patient.active ? 'Inativar' : 'Ativar'}</button><button type="button" onClick={() => onEdit(row.patient)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-black text-slate-600 hover:bg-slate-100"><Pencil size={14} /> Editar</button><button type="button" onClick={() => onDelete(row.patient)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-black text-slate-500 hover:bg-rose-50 hover:text-rose-700"><Trash2 size={14} /> Excluir</button></>}
            </div>
          </details>
        </div>
      </div>; })}</div>
...
function PsychologySettingsView({ store, settings, patients, sessionPackages, remoteMode, remoteSnapshotReady = false, capabilities, onUpdatePackage, onUpdate, onUpdateServices, onUpdateLocation, onCreateLocation, onSetLocationColor, onSetPrimary, onSetActive, onSetColor, onRestoreColors, preview, hiddenCancelledEventCount, onRestoreHiddenCancelled, previewLoading, previewLoadError, onActivatePreview, onEndPreview, onGenerateBackup }: PsychologySettingsViewProps) {
  ...
  return <div className={psychologySettingsContainerClass} data-testid="psychology-settings"><nav className="overflow-x-auto rounded-2xl border border-slate-200 bg-slate-50 p-1.5 shadow-sm" data-testid="psychology-settings-tabs"><div className="flex min-w-max gap-1.5 sm:grid sm:min-w-0 sm:grid-cols-3 xl:grid-cols-5" role="tablist" aria-label="Áreas de ajustes da Psicologia">{tabItems.map(tab => <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} data-testid={`psychology-settings-tab-${tab.id}`} onClick={() => setActiveTab(tab.id)} className={`${psychologySettingsTabClass} ${activeTab === tab.id ? 'border-violet-400 bg-violet-50 text-violet-900 ring-2 ring-violet-100' : 'border-slate-200 bg-white text-slate-600 hover:border-violet-200 hover:bg-slate-50 hover:text-violet-800'}`}><span className={psychologySettingsTabLabelClass}>{tab.label}</span><span className={`${psychologySettingsTabDescriptionClass} ${activeTab === tab.id ? 'text-violet-700' : 'text-slate-400'}`}>{tab.description}</span></button>)}</div></nav>{notice && <div role="status" className="flex w-full items-start justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-800"><span>{notice}</span><button type="button" aria-label="Fechar aviso" onClick={() => setNotice('')} className="shrink-0 text-emerald-700 hover:text-emerald-900"><X size={16} /></button></div>}{remoteMode && !settingsCanEdit && <RemoteCapabilityNotice title="Ajustes remotos em somente leitura" text="O provider autenticado não autorizou alterações de configuração neste contexto. Nenhuma tentativa será confirmada localmente." />}{activeTab !== 'online' && <SettingsPageHeader title={activeTabDefinition.label} description={activeTabDefinition.description} />}
    ...
    {activeTab === 'online' && <div data-testid="psychology-settings-panel-online">{remoteMode && !psychologyCapabilityIsAvailable(capabilities, 'onlineBooking') ? <><RemoteCapabilityNotice title="Agendamento online remoto indisponível" text="A fonte autoritativa permanece no servidor público, mas não existe endpoint autenticado de configuração para este painel. Nenhuma alteração em localStorage é feita nesta sessão." /><PublicBookingSettingsPanel remoteServices={store.services} remoteLocations={store.locations} onSaveRemoteServices={onUpdateServices} /></> : remoteMode ? <><RemoteCapabilityNotice title="Agendamento online remoto somente leitura" text="A configuração pública não pode ser alterada por este painel nesta etapa." /><PublicBookingSettingsPanel remoteServices={store.services} remoteLocations={store.locations} onSaveRemoteServices={onUpdateServices} /></> : <PublicBookingSettingsPanel />}</div>}

    {activeTab === 'backup' && <div className="space-y-3" data-testid="psychology-settings-panel-backup"><DoctoraliaFirstPartyPreviewPanel remoteMode={remoteMode} snapshotReady={remoteSnapshotReady} store={store} /><PsychologyImportExport store={store} onGenerateBackup={onGenerateBackup} /><section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" data-testid="psychology-reminders-settings"><h3 className="text-base font-black">Lembretes</h3><label className="mt-3 flex items-center gap-3 text-sm font-bold"><input type="checkbox" checked={settings.reminders.enabled} onChange={event => onUpdate({ reminders: { ...settings.reminders, enabled: event.target.checked } })} disabled={remoteMode && !settingsCanEdit} /> Preparar lembretes (sem envio nesta etapa)</label><p className="mt-2 text-xs text-slate-500">Nenhuma mensagem ou notificação externa é disparada nesta fundação.</p></section></div>}
