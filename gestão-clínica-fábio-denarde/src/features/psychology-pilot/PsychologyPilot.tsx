  ...
  const confirmPatientDelete = async () => {
    ...
    if (!isLocalPersistence) {
      ...
      try {
        ...
        const refreshed = await loadRemoteStore();
        const remaining = refreshed?.store.patients.find(patient => patient.id === patientDelete.id);
    ...
    <div className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900" data-testid="psychology-pilot">
      ...
      <div className="psychology-mobile-content flex w-full flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:px-8 lg:py-5">
        ...
        <main className="min-w-0 flex-1">
          ...
          {page === 'settings' && <PsychologySettingsView store={store} settings={store.settings} patients={store.patients} sessionPackages={store.sessionPackages} onUpdatePackage={input => updateStore(upsertPsychologySessionPackage(store, input, undefined))} onUpdate={patch => updateStore(updatePsychologySettings(store, patch))} onUpdateLocation={(id, patch) => updateStore(updatePsychologyLocation(store, id, patch))} onCreateLocation={input => updateStore(createPsychologyLocation(store, input))} onSetLocationColor={(id, color) => updateStore(setPsychologyLocationColor(store, id, color))} onSetPrimary={id => updateStore(setPsychologyPrimaryLocation(store, id))} onSetActive={(id, active) => updateStore(setPsychologyLocationActive(store, id, active))} onSetColor={(category, color) => updateStore(setPsychologyCategoryColor(store, category, color))} onRestoreColors={() => updateStore(restorePsychologyDefaultColors(store))} preview={doctoraliaPreview} hiddenCancelledEventCount={hiddenDoctoraliaCancelledEventIds.length} onRestoreHiddenCancelled={restoreHiddenDoctoraliaCancelledEvents} previewLoading={previewLoading} previewLoadError={previewLoadError} onActivatePreview={activateDoctoraliaPreview} onEndPreview={requestEndDoctoraliaPreview} />}
        </main>
      </div>
      ...
      {patientDialog && <PatientDialogR2F3E value={patientDialog === 'new' ? null : patientDialog} onClose={() => setPatientDialog(null)} onSave={async (input) => { const isNew = patientDialog === 'new'; const nextStore = upsertPsychologyPatient(store, input, isNew ? undefined : patientDialog.id); if (!await updateStore(nextStore)) return; const savedPatient = isNew ? nextStore.patients[nextStore.patients.length - 1] : undefined; setPatientDialog(null); if (savedPatient) setPatientChart(savedPatient); setNotice(isNew ? 'Paciente criado. A ficha está pronta para agendar a primeira sessão.' : 'Paciente atualizado.'); }} />}
      ...
      {sessionDialog && <SessionDialog value={sessionDialog === 'new' ? null : sessionDialog} store={store} settings={store.settings} defaultPatientId={sessionDialog === 'new' ? sessionPatientId : undefined} defaultDate={sessionDialog === 'new' ? sessionDefaults.date : selectedDate} defaultTime={sessionDialog === 'new' ? sessionDefaults.time : undefined} onClose={() => { setSessionDialog(null); setSessionPatientId(undefined); }} onSave={async (input) => { if (!await updateStore(upsertPsychologySession(store, input, sessionDialog === 'new' ? undefined : sessionDialog.id))) return; setSelectedDate(input.date); setSessionDialog(null); setSessionPatientId(undefined); setPage('agenda'); setNotice(sessionDialog === 'new' ? 'Sessão agendada.' : 'Sessão atualizada.'); }} />}
      {newEventDialog && <EventCreationDialog defaults={newEventDialog} store={store} settings={store.settings} onClose={() => setNewEventDialog(null)} onNewPatient={() => setPatientDialog('new')} onSaveSession={async (input) => { if (!await updateStore(upsertPsychologySession(store, input))) return; setSelectedDate(input.date); setNewEventDialog(null); setPage('agenda'); setNotice('Sessão agendada.'); }} onSavePersonal={async (input) => { if (!await updateStore(upsertPsychologyPersonalCommitment(store, input))) return; setSelectedDate(input.date); setNewEventDialog(null); setPage('agenda'); setNotice('Compromisso salvo.'); }} />}
      {recordDialog && <RecordDialog session={recordDialog} patient={patientMap.get(recordDialog.patientId)} existingText={store.sessionRecords.find(record => record.sessionId === recordDialog.id)?.text || ''} onClose={() => setRecordDialog(null)} onSave={async (text) => { if (!await updateStore(savePsychologySessionRecord(store, recordDialog.id, text))) return; setRecordDialog(null); setNotice('Registro da sessão salvo.'); }} />}
      {sessionActions && <SessionActionsDialog session={sessionActions} patient={patientMap.get(sessionActions.patientId)} hasRecord={recordsBySession.has(sessionActions.id)} onClose={() => setSessionActions(null)} onEdit={() => { setSessionDialog(sessionActions); setSessionActions(null); }} onStatus={(status) => { void updateStore(updatePsychologySessionStatus(store, sessionActions.id, status)); setSessionActions(null); }} onRecord={() => { setRecordDialog(sessionActions); setSessionActions(null); }} readOnly={isPreview} />}
    </div>
  );
}
...
function PatientsView({ rows, search, setSearch, onNew, onEdit, onOpen, onDelete, onToggle, preview }: { rows: PsychologyPatientListItem[]; search: string; setSearch: (value: string) => void; onNew: () => void; onEdit: (patient: PsychologyPatient) => void; onOpen: (patient: PsychologyPatient) => void; onDelete: (patient: PsychologyPatient) => void; onToggle: (patient: PsychologyPatient) => void; preview?: PsychologyDoctoraliaPreview | null }) {
  ...
  return <div data-testid="psychology-patients" className="w-full">
    ...
    {filteredRows.length === 0 ? <EmptyState title={search || statusFilter !== 'all' ? 'Nenhum paciente encontrado' : 'Nenhum paciente cadastrado'} text={search ? 'Tente outro nome, telefone, e-mail ou status.' : 'Cadastre somente as informações essenciais para começar.'} action={!search && !preview ? <button type="button" onClick={onNew} className={primaryButton}><Plus size={16} /> Novo paciente</button> : undefined} /> : <div data-testid="psychology-patient-list" className="w-full overflow-hidden bg-white">
      ...
      <div>{filteredRows.map(row => <div key={row.patient.id} data-testid="psychology-patient-list-row" className={`group grid ${PATIENT_LIST_GRID} ${PATIENT_LIST_PADDING} border-b border-slate-100 py-4 transition-colors last:border-b-0 hover:bg-slate-50/70`}>
        ...
        <div className="col-span-2 flex flex-wrap justify-start gap-2 pt-2 text-right md:col-auto md:justify-end md:pt-0"><button type="button" onClick={() => onOpen(row.patient)} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-black text-violet-700 hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"><FileText size={14} /> Abrir ficha</button>{!preview && <><button type="button" onClick={() => onEdit(row.patient)} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-black text-slate-600 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"><Pencil size={14} /> Editar</button><button type="button" onClick={() => onToggle(row.patient)} className="rounded-lg px-2 py-1.5 text-xs font-black text-slate-500 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">{row.patient.active ? 'Inativar' : 'Ativar'}</button><button type="button" onClick={() => onDelete(row.patient)} className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-black text-slate-400 hover:bg-rose-50 hover:text-rose-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500"><Trash2 size={13} /> Excluir</button></>}</div>
      </div>)}</div>
    </div>}
  </div>;
}
...
function PsychologySettingsView({ store, settings, patients, sessionPackages, onUpdatePackage, onUpdate, onUpdateLocation, onCreateLocation, onSetLocationColor, onSetPrimary, onSetActive, onSetColor, onRestoreColors, preview, hiddenCancelledEventCount, onRestoreHiddenCancelled, previewLoading, previewLoadError, onActivatePreview, onEndPreview }: PsychologySettingsViewProps) {
  ...
  const saveServices = () => { onUpdate({ services: serviceDrafts }); setAttendanceSection(null); setNotice('Serviços salvos neste navegador.'); };
  const saveLocation = (id: string, patch: PsychologyLocationInput) => { onUpdateLocation(id, patch); setNotice('Local atualizado neste navegador.'); };
  const addLocation = () => {
  ...
  if (activeTab === 'system') return <div className="space-y-3" data-testid="psychology-settings-panel-system"><section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Aparência</p><h3 className="mt-1 text-base font-black text-slate-900">Cores e preferências</h3><p className="mt-1 text-sm text-slate-500">Personalize a aparência da sua agenda.</p></div><button type="button" onClick={onRestoreColors} className={secondaryButton}>Restaurar padrões</button></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{categoryOrder.map(category => <label key={category} className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold"><span>{PSYCHOLOGY_CATEGORY_LABELS[category]}</span><input type="color" value={settings.colors[category]} onChange={event => onSetColor(category, event.target.value)} aria-label={`Cor ${PSYCHOLOGY_CATEGORY_LABELS[category]}`} className="h-8 w-10" /></label>)}</div></section><section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><h3 className="text-base font-black">Lembretes</h3><label className="mt-3 flex items-center gap-3 text-sm font-bold"><input type="checkbox" checked={settings.reminders.enabled} onChange={event => onUpdate({ reminders: { ...settings.reminders, enabled: event.target.checked } })} /> Preparar lembretes</label><p className="mt-2 text-xs text-slate-500">Nenhuma mensagem externa é disparada nesta etapa.</p></section></div>;
...
function LocationEditor({ location, onSave, onSetColor, onSetPrimary, onSetActive }: { key?: React.Key; location: PsychologyLocation; onSave: (patch: PsychologyLocationInput) => void; onSetColor: (color: string) => void; onSetPrimary: () => void; onSetActive: (active: boolean) => void }) {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black">{location.isPrimary ? 'Local principal' : 'Local presencial'}</p><span className={`mt-1 inline-flex rounded-full px-2 py-1 text-[10px] font-black ${location.active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-200 text-slate-500'}`}>{location.active ? 'Ativo' : 'Inativo'}</span></div><input type="color" value={location.color} onChange={event => onSetColor(event.target.value)} aria-label={`Cor do local ${location.displayName}`} className="h-9 w-12 cursor-pointer rounded-lg border-0 bg-transparent" /></div><label className="mt-3 block text-xs font-bold text-slate-600">Nome<input aria-label={`Nome do local ${location.displayName}`} value={name} onChange={event => setName(event.target.value)} className={`${inputClass} mt-1`} /></label><label className="mt-3 block text-xs font-bold text-slate-600">Endereço (opcional)<input aria-label={`Endereço do local ${location.displayName}`} value={address} onChange={event => setAddress(event.target.value)} className={`${inputClass} mt-1`} /></label><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => onSave({ displayName: name, address })} className={secondaryButton}>Salvar local</button>{location.active && !location.isPrimary && <button type="button" onClick={onSetPrimary} className="rounded-xl border border-violet-200 bg-violet-50 px-3 py-2 text-xs font-black text-violet-700">Definir principal</button>}<button type="button" onClick={() => onSetActive(!location.active)} className="rounded-xl px-3 py-2 text-xs font-black text-slate-500 hover:bg-white">{location.active ? 'Desativar' : 'Ativar'}</button></div></div>;
}

function PatientDialogR2F3E({ value, onClose, onSave }: { value: PsychologyPatient | null; onClose: () => void; onSave: (input: PsychologyPatientInput) => void }) {
  ...
  const save = (event: React.FormEvent) => { event.preventDefault(); const nextErrors = validatePsychologyPatientProfile(form) as Record<string, string>; setErrors(nextErrors); if (Object.keys(nextErrors).length === 0) onSave({ ...form, dateOfBirth: String(form.dateOfBirth || form.birthDate || ''), birthDate: undefined }); };
...
function SessionDialog({ value, store, settings, defaultPatientId, defaultDate, defaultTime, onClose, onSave }: { value: PsychologySession | null; store: PsychologyStore; settings: PsychologySettings; defaultPatientId?: string; defaultDate: string; defaultTime?: string; onClose: () => void; onSave: (input: PsychologySessionInput) => void }) {
...
function RecordDialog({ session, patient, existingText, onClose, onSave }: { session: PsychologySession; patient?: PsychologyPatient; existingText: string; onClose: () => void; onSave: (text: string) => void }) {
