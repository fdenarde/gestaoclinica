...
import {
  AlertTriangle, ArrowLeft, Cake, CalendarDays, Check, CheckCircle2, ChevronLeft, ChevronRight, Clock3, FileText, Menu, Pencil,
...
import PsychologyLoading from './PsychologyLoading';
...
import { getPsychologyPatientListViewModels, type PsychologyPatientListItem } from './psychologyPatientList';
...
import {
  ...
  type PsychologyRepositoryBundle,
...
const viteRuntimeEnv: Record<string, string | boolean | undefined> = (import.meta as ImportMeta & { env?: Record<string, string | boolean | undefined> }).env || {};
const PSYCHOLOGY_PERSISTENCE_MODE: PsychologyPersistenceMode = viteRuntimeEnv.VITE_PSYCHOLOGY_PERSISTENCE_MODE === 'local' ? 'local' : 'real';
...
async function synchronizePsychologyAggregate<T extends { id: string }>(
  ...
  scope: PsychologyRepositoryBundle['scope'],
  ...
  for (const item of nextItems) {
    ...
    if (!current) {
      await repository.upsert(scope, item);
    } else if (JSON.stringify(current) !== JSON.stringify(item)) {
      await repository.update(scope, item.id, item);
    }
  }
  for (const item of currentItems) {
    if (!nextById.has(item.id)) await repository.delete(scope, item.id);
  }
}
...
function MovableDialog({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-950/45" role="dialog" aria-modal="true">
      <section ref={dialogRef} style={positionStyle} className="absolute max-h-[calc(100vh-2rem)] w-[calc(100vw-1.5rem)] max-w-xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
...
function Dialog({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return <MovableDialog title={title} onClose={onClose}>{children}</MovableDialog>;
}
...
async function releaseSubmitLock(result: boolean | Promise<boolean>, unlock: () => void): Promise<void> {
  try {
    if (!await result) unlock();
  } catch {
    unlock();
  }
}
...
export default function PsychologyPilot() {
  ...
  const [remoteRepositories, setRemoteRepositories] = useState<PsychologyRepositoryBundle | null>(null);
  ...
  const remoteLoadVersion = useRef(0);
  const loadRemoteStore = useCallback(async () => {
    if (isLocalPersistence) return null;
    const requestVersion = ++remoteLoadVersion.current;
    const result = await readRealPsychologyStore({ includeOperationalSettings: true });
    if (requestVersion !== remoteLoadVersion.current) return null;
  ...
  useEffect(() => {
    ...
    void loadRemoteStore()
      .catch(cause => {
        if (!disposed) setRemoteLoadError(cause instanceof Error ? cause.message : 'Não foi possível carregar a Psicologia.');
      })
      .finally(() => {
        if (!disposed) setRemoteLoading(false);
      });
    return () => { disposed = true; };
  }, [isLocalPersistence, loadRemoteStore]);
  ...
  const visiblePatients = useMemo(() => {
    ...
    const phoneQuery = normalizePsychologyPhoneForSearch(search);
    const filtered = store.patients.filter(patient => !query || [patient.name, patient.email || '', patient.active ? 'ativo' : 'inativo'].some(value => value.toLocaleLowerCase().includes(query)) || Boolean(phoneQuery && normalizePsychologyPhoneForSearch(patient.phone).includes(phoneQuery)));
    return getPsychologyPatientListViewModels(store, filtered);
  }, [search, store]);
  ...
  const currentPatientDialog = patientDialog && patientDialog !== 'new'
    ? store.patients.find(patient => patient.id === patientDialog.id) || patientDialog
    : patientDialog;
  const currentSessionDialog = sessionDialog && sessionDialog !== 'new'
    ? store.sessions.find(session => session.id === sessionDialog.id) || sessionDialog
    : sessionDialog;
  const currentSessionActions = sessionActions
    ? store.sessions.find(session => session.id === sessionActions.id) || sessionActions
    : null;
  const currentRecordDialog = recordDialog
    ? store.sessions.find(session => session.id === recordDialog.id) || recordDialog
    : null;

  const persistedMutationLocks = useRef(new Set<string>());
  const activeMutation = useRef(false);
  const updateStore: PsychologyStoreChangeHandler = async (next: PsychologyStore): Promise<boolean> => {
    ...
    try {
      ...
      setNotice('O navegador não permitiu preparar a alteração. Nenhuma alteração foi confirmada.');
      return false;
    }
    if (persistedMutationLocks.current.has(serialized)) return false;
    if (activeMutation.current) {
      setNotice('Já existe uma alteração em andamento. Aguarde a confirmação antes de tentar novamente.');
      return false;
    }
    ...
    try {
      ...
      const unsupportedChanges = [
        ['registros clínicos', store.sessionRecords, next.sessionRecords],
        ['financeiro', store.charges, next.charges],
        ['financeiro', store.payments, next.payments],
        ['financeiro', store.expenses, next.expenses],
        ['pacotes', store.sessionPackages, next.sessionPackages],
        ['documentos', store.documents, next.documents],
        ['anexos', store.attachments, next.attachments],
      ].some(([, currentItems, nextItems]) => JSON.stringify(currentItems) !== JSON.stringify(nextItems));
      if (unsupportedChanges) {
        setNotice('Esta ação ainda não está disponível para gravação nesta área da Psicologia.');
        return false;
      }
      const scope = remoteRepositories.scope;
      const patientsChanged = JSON.stringify(store.patients) !== JSON.stringify(next.patients);
      const sessionsChanged = JSON.stringify(store.sessions) !== JSON.stringify(next.sessions);
      const personalAppointmentsChanged = JSON.stringify(store.personalCommitments) !== JSON.stringify(next.personalCommitments);
      const servicesChanged = JSON.stringify(store.services) !== JSON.stringify(next.services);
      const locationsChanged = JSON.stringify(store.locations) !== JSON.stringify(next.locations);
      const settingsChanged = JSON.stringify(store.settings) !== JSON.stringify(next.settings);
      await synchronizePsychologyAggregate(remoteRepositories.patients, scope, store.patients as never, next.patients as never);
      await synchronizePsychologyAggregate(remoteRepositories.sessions, scope, store.sessions as never, next.sessions as never);
      await synchronizePsychologyAggregate(remoteRepositories.personalAppointments, scope, store.personalCommitments as never, next.personalCommitments as never);
      await synchronizePsychologyAggregate(remoteRepositories.services, scope, store.services as never, next.services as never);
      await synchronizePsychologyAggregate(remoteRepositories.locations, scope, store.locations as never, next.locations as never);
      if (settingsChanged) {
        await remoteRepositories.settings.update(scope, 'settings', {
          professionalProfile: next.settings.professionalProfile,
          agenda: next.settings.agenda,
          colors: next.settings.colors,
          reminders: next.settings.reminders,
        } as never);
      }
      const [patients, sessions, personalCommitments, services, locations, settingsRecord] = await Promise.all([
        patientsChanged ? remoteRepositories.patients.list(scope) : Promise.resolve(next.patients),
        sessionsChanged ? remoteRepositories.sessions.list(scope) : Promise.resolve(next.sessions),
        personalAppointmentsChanged ? remoteRepositories.personalAppointments.list(scope) : Promise.resolve(next.personalCommitments),
        servicesChanged ? remoteRepositories.services.list(scope) : Promise.resolve(next.services),
        locationsChanged ? remoteRepositories.locations.list(scope) : Promise.resolve(next.locations),
        settingsChanged ? remoteRepositories.settings.get(scope, 'settings') : Promise.resolve(null),
      ]);
      setRemoteStore({
        ...next,
        patients: patients as typeof next.patients,
        sessions: sessions as typeof next.sessions,
        personalCommitments: personalCommitments as typeof next.personalCommitments,
        services: services as typeof next.services,
        locations: locations as typeof next.locations,
        settings: settingsRecord?.settings || next.settings,
      });
  ...
  const runParentMutation = async (key: string, action: () => boolean | Promise<boolean>): Promise<boolean> => {
  ...
  if (!isLocalPersistence && remoteLoading) {
    return <PsychologyLoading />;
  }

  if (!isLocalPersistence && (remoteLoadError || !remoteStore)) {
    return <PsychologyRemoteError onRetry={() => window.location.reload()} />;
  }
  ...
  const applyPublicBookingQuickAction = async (action: PublicBookingQuickAction) => {
    if (!isLocalPersistence) {
      setNotice('A configuração do Agendamento Online não está disponível no momento.');
      return;
    }
    if (doctoraliaPreview) {
  ...
  const confirmPatientDelete = async (): Promise<boolean> => {
    ...
    return runParentMutation(mutationKey, async () => {
      ...
      if (!result.removed) {
        setPatientDelete(null);
        setNotice(result.assessment.reason || 'Este paciente não pode ser excluído definitivamente. Use Inativar.');
        return false;
      }
      if (await updateStore(result.store)) {
        setPatientDelete(null);
        setPatientChart(null);
        setNotice('Paciente e dados vinculados removidos do ambiente local. Pagamentos realizados foram preservados no Financeiro.');
        return true;
      }
      return false;
    });
    ...
    <div className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900" data-testid="psychology-pilot">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 px-4 py-2.5 backdrop-blur sm:px-6 sm:py-3">
        <div className="flex w-full items-center justify-between gap-4">
          ...
          <div className="flex min-w-0 items-center gap-1.5 sm:gap-3"><span className="rounded-full bg-violet-50 px-2 py-1 text-[9px] font-black tracking-wide text-violet-700 sm:px-3 sm:py-2 sm:text-xs" data-testid="psychology-environment-badge">Psicologia</span><a href="/" className={`${secondaryButton} psychology-desktop-link`}><ArrowLeft size={15} /> Gestão Clínica</a>{page !== 'day' && <button type="button" onClick={() => openPage('day')} aria-label="Voltar para Meu Dia" className="rounded-xl p-2 text-slate-500 hover:bg-slate-100 sm:hidden"><ArrowLeft size={19} /></button>}</div>
        </div>
      </header>
      ...
      <div className="psychology-mobile-content flex w-full flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:px-8 lg:py-5">
        <aside className="hidden md:block lg:w-60 lg:shrink-0">
          <div className="mb-3 rounded-2xl border border-violet-100 bg-violet-50 p-3 text-xs text-violet-900"><p className="font-black">Psicologia</p><p className="mt-1 leading-relaxed text-violet-700">Seus dados estão protegidos e separados dos demais profissionais.</p></div>
          <nav className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-1" aria-label="Navegação da Psicologia">
            {([['day', 'Meu Dia', CalendarDays], ['patients', 'Pacientes', UsersRound], ['agenda', 'Agenda', CalendarDays], ['personal', 'Agenda Pessoal', Menu], ['settings', 'Ajustes', Pencil]] as const).map(([id, label, Icon]) => (
        ...
        <main className="min-w-0 flex-1">
          ...
          {page === 'settings' && <PsychologySettingsView store={store} settings={store.settings} patients={store.patients} sessionPackages={store.sessionPackages} onUpdatePackage={input => runParentMutation(`settings:package:${input.patientId}:${input.name}`, () => updateStore(upsertPsychologySessionPackage(store, input, undefined)))} onUpdate={patch => runParentMutation('settings:update', () => updateStore(updatePsychologySettings(store, patch)))} onUpdateLocation={(id, patch) => runParentMutation(`settings:location:${id}`, () => updateStore(updatePsychologyLocation(store, id, patch)))} onCreateLocation={input => runParentMutation(`settings:new-location:${input.displayName}`, () => updateStore(createPsychologyLocation(store, input)))} onSetLocationColor={(id, color) => runParentMutation(`settings:location-color:${id}:${color}`, () => updateStore(setPsychologyLocationColor(store, id, color)))} onSetPrimary={id => runParentMutation(`settings:primary-location:${id}`, () => updateStore(setPsychologyPrimaryLocation(store, id)))} onSetActive={(id, active) => runParentMutation(`settings:location-active:${id}:${active}`, () => updateStore(setPsychologyLocationActive(store, id, active)))} onSetColor={(category, color) => runParentMutation(`settings:category-color:${category}:${color}`, () => updateStore(setPsychologyCategoryColor(store, category, color)))} onRestoreColors={() => runParentMutation('settings:restore-colors', () => updateStore(restorePsychologyDefaultColors(store)))} preview={doctoraliaPreview} hiddenCancelledEventCount={hiddenDoctoraliaCancelledEventIds.length} onRestoreHiddenCancelled={restoreHiddenDoctoraliaCancelledEvents} previewLoading={previewLoading} previewLoadError={previewLoadError} onActivatePreview={activateDoctoraliaPreview} onEndPreview={requestEndDoctoraliaPreview} />}
        </main>
      </div>
      ...
      <nav className="psychology-bottom-nav md:hidden" aria-label="Navegação móvel da Psicologia" data-testid="psychology-bottom-nav">
        ...
        <button type="button" onClick={() => setMobileMoreOpen(true)} aria-expanded={mobileMoreOpen} aria-haspopup="dialog" className={`psychology-bottom-nav-item ${mobileMoreOpen || ['personal', 'settings'].includes(page) ? 'is-active' : ''}`} data-testid="psychology-more-button">
      ...
      {mobileMoreOpen && <div className="psychology-more-sheet-backdrop md:hidden" role="presentation" onClick={() => setMobileMoreOpen(false)}>
        <section className="psychology-more-sheet" role="dialog" aria-modal="true" aria-label="Mais opções da Psicologia" onClick={event => event.stopPropagation()}>
          ...
          <div className="mt-4 grid gap-2">
            {([['personal', 'Agenda Pessoal', WalletCards], ['settings', 'Ajustes', Pencil]] as const).map(([id, label, Icon]) => (
      ...
      {patientChart && <PsychologyPatientChart store={store} patientId={patientChart.id} previewDetails={doctoraliaPreview?.patientDetailsById.get(patientChart.id)} previewClinicalBackground={doctoraliaPreview?.bundle.clinicalBackgrounds.find(item => item.externalPatientId === doctoraliaPreview.patientDetailsById.get(patientChart.id)?.externalPatientId)} readOnly={isPreview} allowSyntheticLocalTools={isLocalPersistence} onClose={() => setPatientChart(null)} onDelete={requestPatientDelete} onEdit={(patient) => { if (isPreview) { setNotice('Edição desabilitada nesta prévia.'); return; } setPatientChart(null); setPatientDialog(patient); }} onSchedule={(patient) => { if (isPreview) { setNotice('Novas sessões estão desabilitadas nesta prévia.'); return; } setPatientChart(null); setSessionPatientId(patient.id); setSessionDefaults({ date: today(), time: '09:00' }); setSessionDialog('new'); }} onOpenSession={setSessionActions} onStoreChange={updateStore} onStatus={(sessionId, status) => runParentMutation(`status:patient-chart:${sessionId}:${status}`, async () => { const saved = await updateStore(updatePsychologySessionStatus(store, sessionId, status)); if (saved) setNotice('Status da sessão atualizado.'); return saved; })} onRecord={setRecordDialog} />}
...
function sessionColor(session: PsychologySession, settings: PsychologySettings): string {
  const location = locationForSession(settings, session);
  return session.modality === 'presencial' && location
    ? colorForPsychologyLocation(location)
    : colorForAgendaCategory(settings.colors, sessionCategory(session));
}

export function PsychologyRemoteError({ onRetry }: { onRetry: () => void }) {
  return <div className="flex min-h-screen items-center justify-center bg-slate-50 p-5" data-testid="psychology-real-error"><section className="w-full max-w-xl rounded-2xl border border-rose-200 bg-white p-6 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.14em] text-rose-700">Acesso indisponível</p><h1 className="mt-2 text-xl font-black text-slate-900">Não foi possível carregar a Psicologia</h1><p className="mt-2 text-sm leading-relaxed text-slate-600">Verifique sua sessão e tente novamente. Nenhum dado local foi usado como substituto.</p><button type="button" onClick={onRetry} className={`${primaryButton} mt-4`}>Tentar novamente</button></section></div>;
}

export function DayView({ date, setDate, store, sessions, settings, onSchedule, onPersonal, onOpenSession }: { date: string; setDate: (date: string) => void; store: PsychologyStore; sessions: PsychologySession[]; settings: PsychologySettings; onSchedule: () => void; onPersonal: () => void; onOpenSession: (session: PsychologySession) => void }) {
  const patientMap = useMemo(() => new Map(store.patients.map(patient => [patient.id, patient])), [store.patients]);
  ...
  return <div data-testid="psychology-day" data-testid-dashboard="psychology-my-day-dashboard" className="space-y-5">
    ...
    <div className="grid gap-4 lg:grid-cols-3" data-testid="psychology-operational-panels">
      ...
      <section className={cardClass} data-testid="psychology-pendencies"><div className="flex items-center justify-between"><h3 className="text-lg font-black text-slate-900">Pendências</h3><AlertTriangle size={18} className="text-amber-600" /></div><div className="mt-3 space-y-2 text-sm"><PendingRow label="Sem próxima sessão" value={patientsWithoutNextSession.length} /></div></section>
    ...
    {alerts.length > 0 && <section className="space-y-2" data-testid="psychology-alerts"><h3 className="text-lg font-black text-slate-900">Alertas</h3>{alerts.slice(0, 6).map((alert, index) => <div key={`${alert.text}-${index}`} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-bold text-slate-700"><AlertTriangle size={16} className="shrink-0" />{alert.text}</div>)}</section>}
...
export function DateToolbar({ date, setDate }: { date: string; setDate: (date: string) => void }) {
...
function SessionCard({ session, settings, patient, hasRecord, onEdit, onStatus, onRecord }: { session: PsychologySession; settings: PsychologySettings; patient?: PsychologyPatient; hasRecord: boolean; onEdit: () => void; onStatus: (status: PsychologySessionStatus) => boolean | Promise<boolean>; onRecord: () => void }) {
...
const PATIENT_LIST_GRID = 'grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1 md:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_minmax(0,1.15fr)_minmax(0,.6fr)_minmax(200px,1.6fr)] md:gap-2 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,.9fr)_minmax(0,1fr)_minmax(0,.85fr)_minmax(0,1fr)_minmax(0,1.1fr)_minmax(0,.6fr)_minmax(250px,2.2fr)] xl:gap-3';
const PATIENT_LIST_PADDING = 'px-3';

export function PatientsView({ rows, search, setSearch, onNew, onEdit, onOpen, onDelete, onToggle, preview }: { rows: PsychologyPatientListItem[]; search: string; setSearch: (value: string) => void; onNew: () => void; onEdit: (patient: PsychologyPatient) => void; onOpen: (patient: PsychologyPatient) => void; onDelete: (patient: PsychologyPatient) => void; onToggle: (patient: PsychologyPatient) => boolean | Promise<boolean>; preview?: PsychologyDoctoraliaPreview | null }) {
  const [statusFilter, setStatusFilter] = useState<'all' | 'active' | 'inactive' | 'review'>('all');
  const [pendingPatientId, setPendingPatientId] = useState<string | null>(null);
  const pendingPatientRef = useRef(false);
  const [toggleError, setToggleError] = useState('');
  const submitToggle = async (patient: PsychologyPatient) => {
    if (pendingPatientRef.current) return;
    pendingPatientRef.current = true;
    setPendingPatientId(patient.id);
    setToggleError('');
    try {
      if (!await onToggle(patient)) setToggleError('Não foi possível atualizar o status do paciente. Tente novamente.');
    } catch {
      setToggleError('Não foi possível atualizar o status do paciente. Tente novamente.');
    } finally {
      pendingPatientRef.current = false;
      setPendingPatientId(null);
    }
  };
  const filteredRows = rows.filter(row => {
    if (!preview || statusFilter === 'all') return true;
    const review = Boolean(preview.patientDetailsById.get(row.patient.id)?.reviewReason);
    if (statusFilter === 'active') return row.patient.active;
    if (statusFilter === 'review') return review;
    return !row.patient.active && !review;
  });
  const previewCounts = preview?.bundle.patientCounts;
  return <div data-testid="psychology-patients" className="w-full">
    {preview && <div className="mb-5 rounded-2xl border border-violet-200 bg-violet-50/70 p-4" data-testid="doctoralia-preview-patient-summary"><p className="text-xs font-black uppercase tracking-[0.14em] text-violet-700">Pacientes na prévia local</p><p className="mt-1 text-lg font-black text-violet-950">{previewCounts?.initiallyImportable || rows.length} pacientes</p><p className="mt-1 text-sm font-bold text-violet-800">{previewCounts?.activeByFutureEvidence || 0} ativos · {previewCounts?.inactiveReview || 0} inativos/revisão</p><div className="mt-3 flex flex-wrap gap-2" role="group" aria-label="Filtro de status dos pacientes">{([['all', 'Todos'], ['active', 'Ativos'], ['inactive', 'Inativos'], ['review', 'Revisar status']] as const).map(([value, label]) => <button type="button" key={value} onClick={() => setStatusFilter(value)} className={`rounded-xl px-3 py-2 text-xs font-black ${statusFilter === value ? 'bg-violet-700 text-white' : 'bg-white text-violet-800'}`}>{label}</button>)}</div></div>}
    <div className="mb-5 flex flex-col gap-3 sm:flex-row">
      <label className="relative flex-1"><Search size={17} className="absolute left-3 top-3.5 text-slate-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Buscar paciente por nome, telefone, e-mail ou status" className={`${inputClass} pl-10`} /></label>
      <button type="button" onClick={onNew} disabled={Boolean(preview)} className={primaryButton}><Plus size={17} /> Novo paciente</button>
    </div>
    {filteredRows.length === 0 ? <EmptyState title={search || statusFilter !== 'all' ? 'Nenhum paciente encontrado' : 'Nenhum paciente cadastrado'} text={search ? 'Tente outro nome, telefone, e-mail ou status.' : 'Cadastre somente as informações essenciais para começar.'} action={!search && !preview ? <button type="button" onClick={onNew} className={primaryButton}><Plus size={16} /> Novo paciente</button> : undefined} /> : <div data-testid="psychology-patient-list" className="w-full overflow-hidden bg-white">
      <div data-testid="psychology-patient-list-header" role="row" className={`hidden md:grid ${PATIENT_LIST_GRID} ${PATIENT_LIST_PADDING} border-b border-slate-200 py-3 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400`}>
        <span className="text-left">Paciente</span><span className="text-left">Telefone</span><span className="hidden text-left xl:block">E-mail</span><span className="hidden text-left xl:block">Última sessão</span><span className="text-left">Próxima sessão</span><span className="hidden text-left xl:block">Modalidade / local</span><span className="text-center">Status</span><span className="text-right">Ações</span>
      </div>
      <div>{filteredRows.map(row => <div key={row.patient.id} data-testid="psychology-patient-list-row" className={`group grid ${PATIENT_LIST_GRID} ${PATIENT_LIST_PADDING} border-b border-slate-100 py-4 transition-colors last:border-b-0 hover:bg-slate-50/70`}>
        <button type="button" onClick={() => onOpen(row.patient)} aria-label={`Abrir ficha completa de ${row.patient.name}`} className="min-w-0 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-violet-500 md:truncate"><span className="block truncate text-sm font-black text-slate-900 md:text-base">{row.patient.name}</span></button>
        <div className="min-w-0 text-left text-sm text-slate-600"><span className="mr-2 text-[10px] font-black uppercase tracking-wide text-slate-400 md:hidden">Telefone</span><span className="whitespace-nowrap">{row.phone}</span></div>
        <div className="hidden min-w-0 truncate text-left text-sm text-slate-600 xl:block">{row.email}</div>
        <div className="hidden text-left text-sm text-slate-600 xl:block">{row.lastSession}</div>
        <div className="min-w-0 text-left text-sm text-slate-700"><span className="mr-2 text-[10px] font-black uppercase tracking-wide text-slate-400 md:hidden">Próxima</span><span className="whitespace-nowrap md:whitespace-normal">{row.nextSession}</span></div>
        <div className="block min-w-0 text-sm text-slate-600 md:hidden xl:block"><span className="mr-2 text-[10px] font-black uppercase tracking-wide text-slate-400 md:hidden">Modalidade</span><span>{row.modalityLocation}</span></div>
        <div className="col-start-2 row-start-1 self-start text-center md:col-auto md:row-auto"><span className={`inline-flex rounded-full px-2 py-1 text-[11px] font-black ${row.patient.active ? 'bg-emerald-50 text-emerald-700' : preview && preview.patientDetailsById.get(row.patient.id)?.reviewReason ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>{row.patient.active ? 'Ativo' : preview && preview.patientDetailsById.get(row.patient.id)?.reviewReason ? 'Revisar status' : 'Inativo'}</span></div>
        <div className="col-span-2 flex flex-wrap justify-start gap-2 pt-2 text-right md:col-auto md:justify-end md:pt-0"><button type="button" onClick={() => onOpen(row.patient)} disabled={Boolean(pendingPatientId)} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-black text-violet-700 hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 disabled:opacity-50"><FileText size={14} /> Abrir ficha</button>{!preview && <><button type="button" onClick={() => onEdit(row.patient)} disabled={Boolean(pendingPatientId)} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-black text-slate-600 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 disabled:opacity-50"><Pencil size={14} /> Editar</button><button type="button" onClick={() => submitToggle(row.patient)} disabled={Boolean(pendingPatientId)} className="rounded-lg px-2 py-1.5 text-xs font-black text-slate-500 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 disabled:opacity-50">{pendingPatientId === row.patient.id ? 'Salvando…' : row.patient.active ? 'Inativar' : 'Ativar'}</button><button type="button" onClick={() => onDelete(row.patient)} disabled={Boolean(pendingPatientId)} className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-black text-slate-400 hover:bg-rose-50 hover:text-rose-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 disabled:opacity-50"><Trash2 size={13} /> Excluir</button></>}</div>{toggleError && <p role="alert" className="col-span-2 pt-2 text-left text-xs font-bold text-rose-700 md:col-start-1">{toggleError}</p>}
      </div>)}</div>
    </div>}
  </div>;
}
...
export function AgendaView({ sessions, personalCommitments, patientMap, settings, publicBookingSettings, weekStart, onPreviousWeek, onNextWeek, onToday, onNew, onPublicBookingAction, onOpenSession, onRemoveCancelled, onOpenPersonal }: { sessions: PsychologySession[]; personalCommitments: PsychologyPersonalCommitment[]; patientMap: Map<string, PsychologyPatient>; settings: PsychologySettings; publicBookingSettings?: PublicBookingSettings; weekStart: Date; onPreviousWeek: () => void; onNextWeek: () => void; onToday: () => void; onNew: (date?: string, time?: string, kind?: NewEventKind) => void; onPublicBookingAction: (action: PublicBookingQuickAction) => void; onOpenSession: (session: PsychologySession) => void; onRemoveCancelled?: (session: PsychologySession) => void; onOpenPersonal: (item: PsychologyPersonalCommitment) => void }) {
  ...
  const dayEvents = (day: Date, time: string) => {
    ...
    return {
      sessions: sessions.filter(session => session.date === date && session.time === time),
...
export function SessionActionsDialog({ session, patient, hasRecord, onClose, onEdit, onStatus, onRecord, onDelete, readOnly }: { session: PsychologySession; patient?: PsychologyPatient; hasRecord: boolean; onClose: () => void; onEdit: () => void; onStatus: (status: PsychologySessionStatus) => boolean | Promise<boolean>; onRecord: () => void; onDelete: () => void; readOnly?: boolean }) {
  ...
  const submitStatus = async (status: PsychologySessionStatus) => {
    ...
    setMutationError('');
    try {
      if (!await onStatus(status)) setMutationError('Não foi possível atualizar o status. A sessão continua aberta para tentar novamente.');
    } catch {
      setMutationError('Não foi possível atualizar o status. A sessão continua aberta para tentar novamente.');
    } finally {
      pendingStatusRef.current = null;
      setPendingStatus(null);
    }
...
function LegacyPsychologySettingsView({ store, settings, patients, sessionPackages, onUpdatePackage, onUpdate, onUpdateLocation, onCreateLocation, onSetLocationColor, onSetPrimary, onSetActive, onSetColor, onRestoreColors, preview, hiddenCancelledEventCount, onRestoreHiddenCancelled, previewLoading, previewLoadError, onActivatePreview, onEndPreview }: { store: PsychologyStore; settings: PsychologySettings; patients: PsychologyPatient[]; sessionPackages: PsychologyStore['sessionPackages']; onUpdatePackage: (input: PsychologySessionPackageInput) => void; onUpdate: (patch: Partial<PsychologySettings>) => void; onUpdateLocation: (id: string, patch: PsychologyLocationInput) => void; onCreateLocation: (input: PsychologyLocationInput) => void; onSetLocationColor: (id: string, color: string) => void; onSetPrimary: (id: string) => void; onSetActive: (id: string, active: boolean) => void; onSetColor: (category: PsychologyAgendaCategory, color: string) => void; onRestoreColors: () => void; preview?: PsychologyDoctoraliaPreview | null; hiddenCancelledEventCount: number; onRestoreHiddenCancelled: () => void; previewLoading: boolean; previewLoadError: string; onActivatePreview: () => void; onEndPreview: () => void }) {
  ...
  const saveProfile = () => {
    ...
    onUpdate({ professionalProfile: {
      ...profile,
      displayName,
      professionalTitle,
      professionalRegistration: profile.professionalRegistration.trim(),
      clinicDisplayName: profile.clinicDisplayName.trim(),
      name: displayName,
      specialty: professionalTitle,
      crp: profile.professionalRegistration.trim(),
    } });
  };
  const restoreProfile = () => {
    setProfileErrors({});
    onUpdate({ professionalProfile: createDefaultPsychologyProfessionalPresentation(settings.scope) });
  };
  const saveAgenda = () => {
  ...
  const categoryOrder: PsychologyAgendaCategory[] = ['ONLINE', 'PERSONAL', 'MENTORING'];
  return <div className="space-y-5" data-testid="psychology-settings">
    <section className="rounded-2xl border border-amber-200 bg-amber-50/70 p-5 shadow-sm" data-testid="doctoralia-preview-settings">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-amber-800">Doctoralia · prévia local</p><h3 className="mt-1 text-xl font-black text-amber-950">{preview ? 'Prévia ativa e somente leitura' : 'Carregar prévia dos CSVs locais'}</h3><p className="mt-1 max-w-2xl text-sm font-bold text-amber-900">Dados reais carregados somente neste ambiente local. Nenhuma informação foi migrada ou sincronizada.</p></div>{preview ? <button type="button" onClick={onEndPreview} className="rounded-xl border border-amber-300 bg-white px-4 py-3 text-sm font-black text-amber-900 hover:bg-amber-100">Encerrar prévia</button> : <button type="button" onClick={onActivatePreview} disabled={previewLoading} className={primaryButton}>{previewLoading ? 'Carregando prévia…' : 'Ativar prévia Doctoralia'}</button>}</div>
      {preview && <><div className="mt-4 grid gap-3 sm:grid-cols-4"><MetricCard label="Pacientes" value={preview.bundle.patientCounts.initiallyImportable} sub="lista normal" icon={<UsersRound size={17} />} /><MetricCard label="Ativos" value={preview.bundle.patientCounts.activeByFutureEvidence} sub="evidência futura" icon={<CheckCircle2 size={17} />} /><MetricCard label="Inativos" value={preview.bundle.patientCounts.inactiveReview} sub="revisão local" icon={<UsersRound size={17} />} /><MetricCard label="Sessões" value={preview.bundle.appointmentCounts.importable} sub="a partir de 2025" icon={<CalendarDays size={17} />} /></div>{hiddenCancelledEventCount > 0 && <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white/80 p-3"><div><p className="font-black text-slate-800">Consultas canceladas ocultadas</p><p className="mt-1 text-xs font-bold text-slate-500">{hiddenCancelledEventCount} item(ns) somente na Agenda desta prévia.</p></div><button type="button" onClick={onRestoreHiddenCancelled} className={secondaryButton}>Restaurar consultas canceladas ocultadas</button></div>}<div className="mt-4 rounded-xl border border-amber-200 bg-white/70 p-3"><p className="font-black text-amber-950">Não migrados inicialmente · {preview.bundle.notImportedPatients.length}</p><p className="mt-1 text-xs font-bold text-amber-800">Grupo C permanece fora da lista normal e pode ser marcado para revisão apenas nesta tela.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{preview.bundle.notImportedPatients.map(patient => <div key={patient.externalPatientId} className="flex items-center justify-between gap-2 rounded-lg border border-amber-100 bg-white px-3 py-2 text-sm"><span className="min-w-0 truncate font-bold text-slate-800">{patient.name}</span><button type="button" onClick={() => setReviewSelection(current => current.includes(patient.externalPatientId) ? current.filter(id => id !== patient.externalPatientId) : [...current, patient.externalPatientId])} className="shrink-0 rounded-lg px-2 py-1 text-xs font-black text-amber-800 hover:bg-amber-100">{reviewSelection.includes(patient.externalPatientId) ? 'Marcado para revisão' : 'Revisar'}</button></div>)}</div></div></>}
      {!preview && previewLoadError && <p className="mt-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-bold text-rose-800">{previewLoadError}</p>}
    </section>
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" data-testid="psychology-professional-profile"><div className="mb-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-violet-700">Ajustes</p><h3 className="mt-1 text-xl font-black">Perfil profissional</h3><p className="mt-1 text-sm text-slate-500">A identidade abaixo é apenas de apresentação. O vínculo de acesso permanece protegido.</p></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Nome de exibição" error={profileErrors.displayName}><input aria-label="Nome de exibição" maxLength={120} value={profile.displayName} onChange={event => setProfile({ ...profile, displayName: event.target.value })} className={inputClass} placeholder="Ex.: Leila Chaves" /></Field><Field label="Área / título profissional" error={profileErrors.professionalTitle}><input aria-label="Área / título profissional" maxLength={80} value={profile.professionalTitle} onChange={event => setProfile({ ...profile, professionalTitle: event.target.value })} className={inputClass} placeholder="Ex.: Psicóloga" /></Field><Field label="Registro profissional"><input aria-label="Registro profissional" maxLength={40} value={profile.professionalRegistration} onChange={event => setProfile({ ...profile, professionalRegistration: event.target.value })} className={inputClass} placeholder="Ex.: CRP 00/00000" /></Field><Field label="Nome da clínica (opcional)"><input aria-label="Nome da clínica" maxLength={120} value={profile.clinicDisplayName} onChange={event => setProfile({ ...profile, clinicDisplayName: event.target.value })} className={inputClass} placeholder="Ex.: Gestão Clínica" /></Field><Field label="E-mail profissional"><input type="email" value={profile.email} onChange={event => setProfile({ ...profile, email: event.target.value })} className={inputClass} /></Field><Field label="Telefone profissional"><input value={formatPsychologyPhoneInput(profile.phone)} onChange={event => setProfile({ ...profile, phone: formatPsychologyPhoneInput(event.target.value) })} className={inputClass} /></Field></div><div className="mt-5 rounded-2xl border border-violet-200 bg-violet-50/70 p-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-violet-700">Prévia no sistema</p><p className="mt-2 text-[10px] font-black uppercase tracking-[0.18em] text-violet-700">Gestão Clínica</p><p className="break-words text-base font-black text-slate-900">{profile.displayName.trim() || 'Profissional'} — {profile.professionalTitle.trim() || 'Psicologia'}</p></div><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={saveProfile} className={primaryButton}>Salvar alterações</button><button type="button" onClick={restoreProfile} className={secondaryButton}>Restaurar dados do perfil</button></div></section>
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4"><h3 className="text-xl font-black">Agenda</h3><p className="mt-1 text-sm text-slate-500">Preferências simples para o atendimento local.</p></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Duração padrão"><input type="number" min="1" value={agenda.defaultDurationMinutes} onChange={event => setAgenda({ ...agenda, defaultDurationMinutes: Number(event.target.value) })} className={inputClass} /></Field><Field label="Intervalo entre sessões"><input type="number" min="0" value={agenda.intervalMinutes} onChange={event => setAgenda({ ...agenda, intervalMinutes: Number(event.target.value) })} className={inputClass} /></Field></div><PsychologyAvailabilitySettings weeklyAvailability={agenda.weeklyAvailability} onChange={weeklyAvailability => setAgenda({ ...agenda, weeklyAvailability })} /><PsychologyDaypartSettings dayParts={agenda.dayParts} onChange={dayParts => setAgenda({ ...agenda, dayParts })} />{agendaError && <p className="mt-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-bold text-rose-700" role="alert">{agendaError}</p>}<button type="button" onClick={saveAgenda} className={`${primaryButton} mt-4`}>Salvar agenda</button></section>
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4"><h3 className="text-xl font-black">Atendimento / Locais</h3><p className="mt-1 text-sm text-slate-500">Cadastre quantos locais precisar. O vínculo das sessões usa o local estável, não o nome.</p></div><div className="grid gap-4 md:grid-cols-2">{settings.locations.map(location => <LocationEditorCanonical key={location.id} location={location} onSave={patch => onUpdateLocation(location.id, patch)} onSetColor={color => onSetLocationColor(location.id, color)} onSetPrimary={() => onSetPrimary(location.id)} onSetActive={active => onSetActive(location.id, active)} />)}</div><div className="mt-5 rounded-2xl border border-dashed border-violet-200 bg-violet-50/60 p-4"><p className="font-black text-violet-950">Adicionar local</p><div className="mt-3 grid gap-3 md:grid-cols-[1fr_1fr_auto] md:items-end"><Field label="Nome"><input aria-label="Nome do novo local" value={newLocation.displayName} onChange={event => setNewLocation({ ...newLocation, displayName: event.target.value })} className={inputClass} placeholder="Ex.: Clínica Praia da Costa" /></Field><Field label="Endereço completo (opcional)"><input aria-label="Endereço do novo local" value={newLocation.address} onChange={event => setNewLocation({ ...newLocation, address: event.target.value })} className={inputClass} placeholder="Rua, número, sala" /></Field><button type="button" onClick={() => { if (!newLocation.displayName?.trim()) return; onCreateLocation(newLocation); setNewLocation({ displayName: '', address: '', color: '#DC2626', active: true, isPrimary: false }); }} className={primaryButton}><Plus size={16} /> Adicionar local</button></div><label className="mt-3 flex items-center gap-2 text-sm font-bold text-violet-900"><input type="checkbox" checked={Boolean(newLocation.isPrimary)} onChange={event => setNewLocation({ ...newLocation, isPrimary: event.target.checked })} /> Definir como principal</label></div></section>
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4"><h3 className="text-xl font-black">Serviços</h3><p className="mt-1 text-sm text-slate-500">O serviço é separado do local/modalidade.</p></div><div className="space-y-2">{settings.services.map(service => <div key={service.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3"><div><p className="font-black">{service.name}</p><p className="text-xs text-slate-500">{service.defaultDurationMinutes} min · {service.defaultPrice ? `R$ ${service.defaultPrice.toFixed(2)}` : 'valor a definir'} · {service.modality === 'BOTH' ? 'Online ou presencial' : service.modality}</p></div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-black text-emerald-700">{service.active ? 'Ativo' : 'Inativo'}</span></div>)}</div><div className="mt-4 flex flex-col gap-2 sm:flex-row"><input value={newServiceName} onChange={event => setNewServiceName(event.target.value)} placeholder="Nome do novo serviço" className={`${inputClass} flex-1`} /><button type="button" onClick={addService} className={secondaryButton}>Adicionar serviço</button></div></section>
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h3 className="text-xl font-black">Cores</h3><p className="mt-1 text-sm text-slate-500">Tipos e locais têm cores independentes. A Agenda deriva automaticamente fundo pastel, borda suave e texto com contraste.</p></div><button type="button" onClick={onRestoreColors} className={secondaryButton}>Restaurar padrões</button></div><p className="mb-2 text-xs font-black uppercase tracking-[0.14em] text-slate-500">Tipos</p><div className="grid gap-3 sm:grid-cols-3">{categoryOrder.map(category => <label key={category} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-3"><span className="font-bold">{PSYCHOLOGY_CATEGORY_LABELS[category]}</span><input type="color" value={settings.colors[category]} onChange={event => onSetColor(category, event.target.value)} className="h-9 w-12 cursor-pointer rounded-lg border-0 bg-transparent" aria-label={`Cor ${PSYCHOLOGY_CATEGORY_LABELS[category]}`} /></label>)}</div><p className="mb-2 mt-5 text-xs font-black uppercase tracking-[0.14em] text-slate-500">Locais presenciais</p><div className="grid gap-3 sm:grid-cols-2">{settings.locations.map(location => <label key={location.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-3"><span className="font-bold">{location.displayName}</span><input type="color" value={location.color} onChange={event => onSetLocationColor(location.id, event.target.value)} className="h-9 w-12 cursor-pointer rounded-lg border-0 bg-transparent" aria-label={`Cor do local ${location.displayName}`} /></label>)}</div><div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/60 p-4" data-testid="psychology-color-preview"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Prévia suave na Agenda</p><p className="mt-1 text-xs text-slate-500">A cor-base escolhida acima aparece assim nos cards, na legenda e nos chips.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{[
      { label: 'Online', style: agendaCategoryVisualStyle('ONLINE', settings) },
      { label: 'Presencial', style: agendaCategoryVisualStyle('PRESENTIAL_PRIMARY', settings) },
      { label: 'Pessoal', style: agendaCategoryVisualStyle('PERSONAL', settings) },
      { label: 'Mentoria', style: agendaCategoryVisualStyle('MENTORING', settings) },
      { label: 'Terapia de Casal', style: resolvePsychologyAgendaEventStyle({ source: 'SESSION', colors: settings.colors, modality: 'presencial', serviceName: 'Terapia de Casal' }) },
      { label: 'Agenda pessoal', style: resolvePsychologyAgendaEventStyle({ source: 'PERSONAL_AGENDA', colors: settings.colors }) },
    ].map(item => <div key={item.label} className="rounded-xl border px-3 py-2 shadow-sm" style={{ backgroundColor: item.style.backgroundColor, borderColor: item.style.borderColor, color: item.style.textColor }}><div className="flex items-center justify-between gap-2"><span className="text-xs font-black">09:00 · {item.label}</span><span className="rounded-full border px-1.5 py-0.5 text-[9px] font-black" style={item.style.chipStyle}>Agendada</span></div><p className="mt-1 truncate text-xs font-black">Paciente de exemplo</p></div>)}</div></div></section>
    <section className="rounded-2xl border border-violet-200 bg-violet-50/60 p-5 shadow-sm" data-testid="psychology-package-settings"><div className="mb-4"><h3 className="text-xl font-black text-violet-950">Acompanhamento / Pacotes opcionais</h3><p className="mt-1 text-sm text-violet-800">Use somente quando o paciente tiver um pacote configurado. A quantidade é livre e não cria regra financeira automática.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Field label="Paciente"><select value={newPackage.patientId} onChange={event => setNewPackage({ ...newPackage, patientId: event.target.value })} className={inputClass}><option value="">Selecione um paciente</option>{patients.filter(patient => patient.active).map(patient => <option key={patient.id} value={patient.id}>{patient.name}</option>)}</select></Field><Field label="Nome"><input value={newPackage.name} onChange={event => setNewPackage({ ...newPackage, name: event.target.value })} className={inputClass} /></Field><Field label="Total de sessões"><input type="number" min="1" value={newPackage.totalSessions} onChange={event => setNewPackage({ ...newPackage, totalSessions: Number(event.target.value) })} className={inputClass} /></Field><Field label="Sessões utilizadas"><input type="number" min="0" value={newPackage.usedSessions || 0} onChange={event => setNewPackage({ ...newPackage, usedSessions: Number(event.target.value) })} className={inputClass} /></Field></div><div className="mt-3 flex flex-wrap items-end gap-3"><Field label="Início"><input type="date" value={newPackage.startDate} onChange={event => setNewPackage({ ...newPackage, startDate: event.target.value })} className={inputClass} /></Field><button type="button" onClick={() => { onUpdatePackage(newPackage); setNewPackage({ ...newPackage, name: 'Pacote de sessões', usedSessions: 0 }); }} className={primaryButton}>Adicionar pacote</button></div>{sessionPackages.length > 0 && <div className="mt-4 grid gap-2 sm:grid-cols-2">{sessionPackages.map(item => <div key={item.id} className="rounded-xl border border-violet-200 bg-white px-3 py-2 text-sm"><span className="font-black">{patients.find(patient => patient.id === item.patientId)?.name || 'Paciente'}</span><span className="ml-2 text-slate-500">{item.name} · {item.usedSessions}/{item.totalSessions}</span></div>)}</div>}</section>
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-xl font-black">Financeiro</h3><p className="mt-2 text-sm text-slate-600">A fundação já separa cobrança de pagamento por paciente, sessão e serviço. O módulo completo de recebimentos, saldo e relatórios financeiros será implementado em etapa própria.</p><span className="mt-3 inline-flex rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">Fundação preparada · sem lançamentos nesta etapa</span></section>
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-xl font-black">Lembretes</h3><label className="mt-4 flex items-center gap-3 text-sm font-bold"><input type="checkbox" checked={settings.reminders.enabled} onChange={event => onUpdate({ reminders: { ...settings.reminders, enabled: event.target.checked } })} /> Preparar lembretes (sem envio nesta etapa)</label><p className="mt-2 text-xs text-slate-500">Nenhuma mensagem ou notificação externa é disparada nesta fundação local.</p></section>
  </div>;
}

type PsychologySettingsTab = 'profile' | 'attendance' | 'agenda' | 'online' | 'messages' | 'system';
...
type PsychologySettingsViewProps = {
  ...
  onUpdatePackage: (input: PsychologySessionPackageInput) => boolean | Promise<boolean>;
  onUpdate: (patch: Partial<PsychologySettings>) => boolean | Promise<boolean>;
  onUpdateLocation: (id: string, patch: PsychologyLocationInput) => boolean | Promise<boolean>;
  onCreateLocation: (input: PsychologyLocationInput) => boolean | Promise<boolean>;
  onSetLocationColor: (id: string, color: string) => boolean | Promise<boolean>;
  onSetPrimary: (id: string) => boolean | Promise<boolean>;
  onSetActive: (id: string, active: boolean) => boolean | Promise<boolean>;
  onSetColor: (category: PsychologyAgendaCategory, color: string) => boolean | Promise<boolean>;
  onRestoreColors: () => boolean | Promise<boolean>;
  ...
  onActivatePreview: () => void | Promise<void>;
  onEndPreview: () => void;
};
...
export function PsychologySettingsView({ store, settings, patients, sessionPackages, onUpdatePackage, onUpdate, onUpdateLocation, onCreateLocation, onSetLocationColor, onSetPrimary, onSetActive, onSetColor, onRestoreColors, preview, hiddenCancelledEventCount, onRestoreHiddenCancelled, previewLoading, previewLoadError, onActivatePreview, onEndPreview }: PsychologySettingsViewProps) {
  ...
  const saveProfile = async () => {
    ...
    setMutationError('');
    setNotice('Salvando perfil…');
    try {
      const saved = await onUpdate({ professionalProfile: { ...profile, displayName, professionalTitle, professionalRegistration: profile.professionalRegistration.trim(), clinicDisplayName: profile.clinicDisplayName.trim(), name: displayName, specialty: professionalTitle, crp: profile.professionalRegistration.trim() } });
      if (!saved) {
        setNotice('');
        setMutationError('Não foi possível salvar o perfil. Tente novamente.');
        return;
      }
      setProfileEditing(false);
      setNotice('Perfil profissional salvo neste navegador.');
    } catch {
      setNotice('');
      setMutationError('Não foi possível salvar o perfil. Tente novamente.');
    } finally {
  ...
  const saveAgenda = async () => {
    ...
    if (invalid) { setAgendaError('Corrija os períodos: o fim precisa ser depois do início.'); return; }
    setAgendaError('');
    setPendingAction('agenda');
    const saved = await onUpdate({ agenda });
    setPendingAction('');
    if (!saved) return;
    setAgendaEditing(false);
    setNotice('Configuração da Agenda salva neste navegador.');
  };
  const addService = async () => {
    ...
    const saved = await onUpdate({ services: [...settings.services, { id: `service-${Date.now()}`, professionalId: settings.scope.professionalId, context: PSYCHOLOGY_CONTEXT, name, defaultDurationMinutes: agenda.defaultDurationMinutes, defaultPrice: 0, modality: 'BOTH', active: true, createdAt: now, updatedAt: now }] });
    setPendingAction('');
    if (!saved) return;
    setNewServiceName('');
    setNotice('Serviço adicionado neste navegador.');
  };
  const saveServices = async () => { setPendingAction('services'); const saved = await onUpdate({ services: serviceDrafts }); setPendingAction(''); if (!saved) return; setAttendanceSection(null); setNotice('Serviços salvos neste navegador.'); };
  const saveLocation = async (id: string, patch: PsychologyLocationInput) => { setPendingAction(`location:${id}`); const saved = await onUpdateLocation(id, patch); setPendingAction(''); if (saved) setNotice('Local atualizado neste navegador.'); return saved; };
  const addLocation = async () => {
    if (!newLocation.displayName?.trim()) return;
    setPendingAction('location:new');
    const saved = await onCreateLocation(newLocation);
    setPendingAction('');
    if (!saved) return;
    setNewLocation({ displayName: '', address: '', color: '#DC2626', active: true, isPrimary: false });
    setNotice('Local adicionado neste navegador.');
  };
  const categoryOrder: PsychologyAgendaCategory[] = ['ONLINE', 'PRESENTIAL_PRIMARY', 'PERSONAL', 'MENTORING'];
  if (activeTab === 'system') return <PsychologySettingsSystemPanel settings={settings} onUpdate={onUpdate} onSetColor={onSetColor} onRestoreColors={onRestoreColors} onBack={() => setActiveTab('agenda')} />;
  const tabItems: Array<{ id: PsychologySettingsTab; label: string; description: string }> = [
    ...
    { id: 'system', label: 'Aparência e Sistema', description: 'Aparência, backup e preferências' },
  ];
...
export function PatientDialogR2F3E({ value, onClose, onSave }: { value: PsychologyPatient | null; onClose: () => void; onSave: (input: PsychologyPatientInput) => boolean | Promise<boolean> }) {
  ...
  useEffect(() => {
    const phone = formatPsychologyPhoneInput(form.phone);
    const responsiblePhone = form.administrativeResponsible ? formatPsychologyPhoneInput(form.administrativeResponsible.phone) : '';
    if (phone === form.phone && responsiblePhone === (form.administrativeResponsible?.phone || '')) return;
    setForm(current => ({ ...current, phone, administrativeResponsible: current.administrativeResponsible ? { ...current.administrativeResponsible, phone: responsiblePhone } : current.administrativeResponsible }));
  }, [form.administrativeResponsible?.phone, form.phone]);
  const save = async (event: React.FormEvent) => { event.preventDefault(); const nextErrors = validatePsychologyPatientProfile(form) as Record<string, string>; setErrors(nextErrors); if (Object.keys(nextErrors).length !== 0 || submitLock.current) return; submitLock.current = true; setSaving(true); await releaseSubmitLock(onSave({ ...form, dateOfBirth: String(form.dateOfBirth || form.birthDate || ''), birthDate: undefined }), () => { submitLock.current = false; setSaving(false); }); };
  return <Dialog title={value ? 'Editar paciente' : 'Novo paciente'} onClose={saving ? () => undefined : onClose}><form onSubmit={save} className="space-y-4"><Field label="Nome completo *" error={errors.name}><input required autoFocus value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} className={inputClass} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Data de nascimento *" error={errors.dateOfBirth || errors.birthDate}><input required type="date" max={referenceDate} value={form.dateOfBirth || form.birthDate || ''} onChange={event => setForm({ ...form, dateOfBirth: event.target.value, birthDate: undefined })} className={inputClass} /></Field><Field label="Telefone *" error={errors.phone}><input required minLength={8} value={form.phone} onChange={event => setForm({ ...form, phone: event.target.value })} className={inputClass} /></Field></div><Field label="E-mail *" error={errors.email}><input required type="email" value={form.email} onChange={event => setForm({ ...form, email: event.target.value })} className={inputClass} /></Field>{showResponsible && <div className="space-y-4 rounded-2xl border border-amber-200 bg-amber-50/70 p-4" data-testid="psychology-patient-responsible"><div><p className="text-sm font-black text-amber-950">Dados do responsável</p><p className="mt-1 text-xs font-semibold text-amber-900">Paciente com menos de 18 anos na data de hoje.</p></div><Field label="Nome completo do responsável *" error={errors['administrativeResponsible.fullName']}><input required value={form.administrativeResponsible?.fullName || ''} onChange={event => setForm({ ...form, administrativeResponsible: { ...emptyResponsible, ...form.administrativeResponsible, fullName: event.target.value } })} className={inputClass} /></Field><Field label="Vínculo / parentesco *" error={errors['administrativeResponsible.relationship']}><input required value={form.administrativeResponsible?.relationship || ''} onChange={event => setForm({ ...form, administrativeResponsible: { ...emptyResponsible, ...form.administrativeResponsible, relationship: event.target.value } })} className={inputClass} /></Field><Field label="Telefone do responsável *" error={errors['administrativeResponsible.phone']}><input required minLength={8} value={form.administrativeResponsible?.phone || ''} onChange={event => setForm({ ...form, administrativeResponsible: { ...emptyResponsible, ...form.administrativeResponsible, phone: event.target.value } })} className={inputClass} /></Field><Field label="E-mail do responsável *" error={errors['administrativeResponsible.email']}><input required type="email" value={form.administrativeResponsible?.email || ''} onChange={event => setForm({ ...form, administrativeResponsible: { ...emptyResponsible, ...form.administrativeResponsible, email: event.target.value } })} className={inputClass} /></Field></div>}{!showResponsible && value?.administrativeResponsible && <p className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600">Dados históricos do responsável permanecem preservados, mas não são exigidos pela idade atual.</p>}<Field label="Modalidade preferencial *"><select required value={form.preferredModality} onChange={event => setForm({ ...form, preferredModality: event.target.value as PsychologyModality })} className={inputClass}><option value="presencial">Presencial</option><option value="online">Online</option></select></Field>{value && <Field label="Observação administrativa"><textarea value={form.administrativeNote} onChange={event => setForm({ ...form, administrativeNote: event.target.value })} className={`${inputClass} min-h-24`} /></Field>}<label className="flex items-center gap-2 text-sm font-bold text-slate-700"><input type="checkbox" checked={form.active} onChange={event => setForm({ ...form, active: event.target.checked })} /> Paciente ativo</label><div className="flex justify-end gap-2 pt-2"><button type="button" onClick={saving ? () => undefined : onClose} className={secondaryButton}>Voltar</button><button type="submit" disabled={saving} className={primaryButton}>{saving ? 'Salvando…' : 'Salvar paciente'}</button></div></form></Dialog>;
}
...
function PatientDialog({ value, onClose, onSave }: { value: PsychologyPatient | null; onClose: () => void; onSave: (input: PsychologyPatientInput) => void }) {
  const [form, setForm] = useState<PsychologyPatientInput>({ name: value?.name || '', birthDate: value?.birthDate || '', phone: formatPsychologyPhoneInput(value?.phone), email: value?.email || '', preferredModality: value?.preferredModality || 'presencial', administrativeNote: value?.administrativeNotes || value?.administrativeNote || '', active: value?.active ?? true, externalReferences: value?.externalReferences });
  const [errors, setErrors] = useState<Partial<Record<keyof PsychologyPatientInput, string>>>({});
  useEffect(() => {
    const phone = formatPsychologyPhoneInput(form.phone);
    if (phone !== form.phone) setForm(current => ({ ...current, phone }));
  }, [form.phone]);
...
export function DeletePatientDialog({ assessment, onClose, onConfirm }: { assessment: ReturnType<typeof getPsychologyPatientDeletionAssessment>; onClose: () => void; onConfirm: () => boolean | Promise<boolean> }) {
  ...
  const submit = async () => {
    if (pendingRef.current || !assessment.canDelete) return;
    pendingRef.current = true;
    setPending(true);
    setError('');
    try {
      if (!await onConfirm()) setError('Não foi possível excluir o paciente. Tente novamente.');
    } catch {
  ...
  return <Dialog title="Excluir definitivamente?" onClose={pending ? () => undefined : onClose}><div className="space-y-4"><p className="text-sm leading-relaxed text-slate-700">Esta ação excluirá definitivamente o paciente e os dados vinculados. Pagamentos já realizados serão preservados no Financeiro sem manter o paciente ativo.</p><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="font-black text-slate-900">Paciente</p><p className="mt-1 text-sm font-bold text-slate-700">{assessment.patient.name}</p><dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-5"><div><dt className="font-bold text-slate-500">Sessões</dt><dd className="font-black">{impact.sessions}</dd></div><div><dt className="font-bold text-slate-500">Registros</dt><dd className="font-black">{impact.records}</dd></div><div><dt className="font-bold text-slate-500">Cobranças</dt><dd className="font-black">{impact.charges}</dd></div><div><dt className="font-bold text-slate-500">Documentos</dt><dd className="font-black">{impact.documents}</dd></div><div><dt className="font-bold text-slate-500">Pacotes</dt><dd className="font-black">{impact.packages}</dd></div></dl><p className="mt-3 text-xs font-bold text-slate-500">Pagamentos encontrados: {impact.payments} · anexos: {impact.attachments}</p></div><div className="flex justify-end gap-2"><button type="button" onClick={pending ? () => undefined : onClose} className={secondaryButton}>Cancelar</button><button type="button" onClick={submit} disabled={pending || !assessment.canDelete} className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-700 px-4 py-3 text-sm font-black text-white shadow-sm transition hover:bg-rose-800 disabled:cursor-not-allowed disabled:opacity-50"><Trash2 size={15} /> {pending ? 'Excluindo…' : 'Excluir definitivamente'}</button>{error && <p role="alert" className="text-sm font-bold text-rose-700">{error}</p>}</div></div></Dialog>;
}
...
function LegacySessionDialog({ value, store, settings, defaultPatientId, defaultDate, defaultTime, onClose, onSave }: { value: PsychologySession | null; store: PsychologyStore; settings: PsychologySettings; defaultPatientId?: string; defaultDate: string; defaultTime?: string; onClose: () => void; onSave: (input: PsychologySessionInput) => boolean | Promise<boolean> }) {
...
export function SessionDialog({ value, store, settings, defaultPatientId, defaultDate, defaultTime, onClose, onSave }: { value: PsychologySession | null; store: PsychologyStore; settings: PsychologySettings; defaultPatientId?: string; defaultDate: string; defaultTime?: string; onClose: () => void; onSave: (input: PsychologySessionInput) => boolean | Promise<boolean> }) {
  ...
  const [saving, setSaving] = useState(false); const submitLock = useRef(false); const save = async (event: React.FormEvent) => { event.preventDefault(); if (submitLock.current) return; const nextError = validatePsychologySession(form, store, { ignoreSessionId: value?.id, requireService: true, checkConflicts: true }); setError(nextError || ''); if (nextError) return; if (!isPsychologyDateTimeWithinAvailability(settings.agenda, form.date, form.time)) { setOutsideAvailability(true); return; } submitLock.current = true; setSaving(true); await releaseSubmitLock(onSave(form), () => { submitLock.current = false; setSaving(false); }); };
...
function RecordDialog({ session, patient, existingText, onClose, onSave }: { session: PsychologySession; patient?: PsychologyPatient; existingText: string; onClose: () => void; onSave: (text: string) => boolean | Promise<boolean> }) {
  const [text, setText] = useState(existingText);
  return <Dialog title="Registrar sessão" onClose={onClose}><div className="mb-4 rounded-xl bg-slate-50 p-3 text-sm text-slate-600"><p><strong>Paciente:</strong> {patient?.name || '—'}</p><p><strong>Data:</strong> {formatShortDate(session.date)} às {session.time}</p><p className="mt-2 text-xs text-slate-500">Este conteúdo fica somente no registro protegido do piloto local e não aparece em listas, agenda ou Meu Dia.</p></div><form onSubmit={event => { event.preventDefault(); if (text.trim()) onSave(text); }} className="space-y-4"><Field label="Registro da sessão"><textarea autoFocus value={text} onChange={event => setText(event.target.value)} placeholder="Escreva o registro da sessão..." className={`${inputClass} min-h-48`} /></Field><div className="flex justify-end gap-2"><button type="button" onClick={onClose} className={secondaryButton}>Voltar</button><button type="submit" disabled={!text.trim()} className={primaryButton}>Salvar</button></div></form></Dialog>;
}
