  ...
  return <div data-testid="psychology-patients" className="w-full">
    ...
    {filteredRows.length === 0 ? <EmptyState title={hasFilters ? 'Nenhum paciente encontrado' : 'Nenhum paciente cadastrado'} text={hasFilters ? 'Ajuste a busca ou os filtros para ver outros pacientes.' : 'Cadastre somente as informações essenciais para começar.'} action={!hasFilters && !preview ? <button type="button" onClick={onNew} className={primaryButton}><Plus size={16} /> Novo paciente</button> : undefined} /> : <div data-testid="psychology-patient-list" className="w-full overflow-hidden bg-white">
      ...
      <div>{filteredRows.map(row => { const rowInReview = isPsychologyPatientInReview(row); return <div key={row.patient.id} data-testid="psychology-patient-list-row" className={`group grid ${PATIENT_LIST_GRID} ${PATIENT_LIST_PADDING} border-b border-slate-100 py-3 transition-colors last:border-b-0 hover:bg-slate-50/70 ${rowInReview ? 'bg-amber-50/30' : ''}`}>
        ...
        <div data-testid="psychology-patient-list-status" className="col-start-3 row-start-1 flex flex-col items-end gap-1 self-start text-center md:col-auto md:row-auto md:items-center"><span className={`inline-flex rounded-full px-2 py-1 text-[11px] font-black ${row.patient.active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{row.patient.active ? 'Ativo' : 'Inativo'}</span><span className="inline-flex rounded-full bg-sky-50 px-2 py-1 text-[10px] font-black text-sky-700">{row.patient.acompanhamentoStatus === 'PAUSADO' ? 'Pausado' : row.patient.acompanhamentoStatus === 'AGUARDANDO_RETORNO' ? 'Aguardando retorno' : row.patient.acompanhamentoStatus === 'ENCERRADO' ? 'Encerrado' : 'Acompanhamento ativo'}</span>{rowInReview && <span className="inline-flex rounded-full bg-amber-50 px-2 py-1 text-[10px] font-black text-amber-700">Em revisão</span>}</div>
<div data-testid="psychology-patient-list-actions" className="col-span-3 row-start-6 flex min-w-0 flex-nowrap items-center justify-end gap-1 whitespace-nowrap pt-2 text-right md:col-auto md:row-auto md:pt-0 xl:col-auto xl:row-auto"><button type="button" onClick={() => onOpen(row.patient)} className="inline-flex items-center gap-1 whitespace-nowrap rounded-lg px-2 py-1.5 text-xs font-black text-violet-700 hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"><FileText size={14} /> Abrir ficha</button><button type="button" onClick={() => onOpenFinance(row.patient)} className="inline-flex items-center gap-1 whitespace-nowrap rounded-lg px-2 py-1.5 text-xs font-black text-emerald-700 hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"><WalletCards size={14} /> Financeiro</button>{!preview && <><button type="button" onClick={() => onToggle(row.patient)} className="rounded-lg px-2 py-1.5 text-xs font-black text-slate-500 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">{row.patient.active ? 'Inativar' : 'Ativar'}</button><button type="button" onClick={() => onEdit(row.patient)} className="inline-flex items-center gap-1 whitespace-nowrap rounded-lg px-2 py-1.5 text-xs font-black text-slate-600 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"><Pencil size={14} /> Editar</button><button type="button" onClick={() => onDelete(row.patient)} className="inline-flex items-center gap-1 whitespace-nowrap rounded-lg px-2 py-1.5 text-xs font-black text-slate-400 hover:bg-rose-50 hover:text-rose-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500"><Trash2 size={13} /> Excluir</button></>}</div>
      </div>; })}</div>
    </div>}
