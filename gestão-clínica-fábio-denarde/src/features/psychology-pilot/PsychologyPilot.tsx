...
function PatientsView({ rows, search, setSearch, onNew, onEdit, onOpen, onDelete, onToggle, onSetReview, onBulkDelete, preview }: { rows: PsychologyPatientListItem[]; search: string; setSearch: (value: string) => void; onNew: () => void; onEdit: (patient: PsychologyPatient) => void; onOpen: (patient: PsychologyPatient) => void; onDelete: (patient: PsychologyPatient) => void; onToggle: (patient: PsychologyPatient) => boolean | Promise<boolean>; onSetReview: (patientIds: string[], inReview: boolean) => boolean | Promise<boolean>; onBulkDelete: (patientIds: string[]) => PsychologyBulkDeletionSummary | Promise<PsychologyBulkDeletionSummary>; preview?: PsychologyDoctoraliaPreview | null }) {
  ...
  const baseRows = useMemo(() => filterPsychologyPatientList(rowsWithPreviewReview, { query: search, lastSession: lastSessionFilter, nextSession: nextSessionFilter, review: reviewFilter }, referenceDate), [lastSessionFilter, nextSessionFilter, referenceDate, reviewFilter, rowsWithPreviewReview, search]);
  const counts = useMemo(() => countPsychologyPatientList(baseRows), [baseRows]);
  const filteredRows = useMemo(() => sortPsychologyPatientList(filterPsychologyPatientList(rowsWithPreviewReview, { query: search, status: statusFilter, lastSession: lastSessionFilter, nextSession: nextSessionFilter, review: reviewFilter }, referenceDate), sortKey, sortDirection), [lastSessionFilter, nextSessionFilter, referenceDate, reviewFilter, rowsWithPreviewReview, search, sortDirection, sortKey, statusFilter]);
  ...
  return <div data-testid="psychology-patients" className="w-full">
    ...
    {filteredRows.length === 0 ? <EmptyState title={hasFilters ? 'Nenhum paciente encontrado' : 'Nenhum paciente cadastrado'} text={hasFilters ? 'Ajuste a busca ou os filtros para ver outros pacientes.' : 'Cadastre somente as informações essenciais para começar.'} action={!hasFilters && !preview ? <button type="button" onClick={onNew} className={primaryButton}><Plus size={16} /> Novo paciente</button> : undefined} /> : <div data-testid="psychology-patient-list" className="w-full overflow-hidden bg-white">
      ...
      <div>{filteredRows.map(row => { const rowInReview = isPsychologyPatientInReview(row); return <div key={row.patient.id} data-testid="psychology-patient-list-row" className={`group grid ${PATIENT_LIST_GRID} ${PATIENT_LIST_PADDING} border-b border-slate-100 py-4 transition-colors last:border-b-0 hover:bg-slate-50/70 ${rowInReview ? 'bg-amber-50/30' : ''}`}>
        ...
        <div className="col-start-3 row-start-1 flex flex-col items-end gap-1 self-start text-center md:col-auto md:row-auto md:items-center"><span className={`inline-flex rounded-full px-2 py-1 text-[11px] font-black ${row.patient.active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{row.patient.active ? 'Ativo' : 'Inativo'}</span>{rowInReview && <span className="inline-flex rounded-full bg-amber-50 px-2 py-1 text-[10px] font-black text-amber-700">Em revisão</span>}</div>
