...
function releaseSubmitLock(result: boolean | Promise<boolean>, unlock: () => void): void {
  void Promise.resolve(result).then(saved => { if (!saved) unlock(); }).catch(unlock);
}
...
export default function PsychologyPilot() {
  ...
  const runParentMutation = async (key: string, action: () => Promise<boolean>): Promise<boolean> => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900" data-testid="psychology-pilot">
      ...
      {sessionDialog && <SessionDialog value={sessionDialog === 'new' ? null : sessionDialog} store={store} settings={store.settings} defaultPatientId={sessionDialog === 'new' ? sessionPatientId : undefined} defaultDate={sessionDialog === 'new' ? sessionDefaults.date : selectedDate} defaultTime={sessionDialog === 'new' ? sessionDefaults.time : undefined} onClose={() => { setSessionDialog(null); setSessionPatientId(undefined); }} onSave={input => runParentMutation(`session:${sessionDialog === 'new' ? 'new' : sessionDialog.id}:${input.patientId}:${input.date}:${input.time}`, async () => { const isNew = sessionDialog === 'new'; if (!await updateStore(upsertPsychologySession(store, input, isNew ? undefined : sessionDialog.id))) return false; setSelectedDate(input.date); setSessionDialog(null); setSessionPatientId(undefined); setPage('agenda'); setNotice(isNew ? 'Sessão agendada.' : 'Sessão atualizada.'); return true; })} />}
