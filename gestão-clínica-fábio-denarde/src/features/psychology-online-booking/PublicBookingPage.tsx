...
import { ArrowLeft, ArrowRight, CalendarDays, CheckCircle2, MapPin, Video } from 'lucide-react';
import { addDaysToDateKey, formatDateKey, publicBookingWeekdayLabel, serviceAllowsModality } from './bookingDomain';
...
export default function PublicBookingPage({ professionalSlug }: { professionalSlug: string }) {
  ...
  if ((step === 4 || step === 5) && datesLoading) return <PublicBookingShell settings={settings}><div className={cardClass}><button type="button" onClick={() => resetFrom(modality === 'PRESENCIAL' ? 3 : 2)} className="mb-5 text-sm font-black text-violet-700"><ArrowLeft size={15} className="mr-1 inline" /> Voltar</button><StepHeader step={progressStep} title="Escolha o dia" text="Calculando somente as datas com horário realmente reservável…" /><div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center text-sm font-semibold text-slate-600">Verificando disponibilidade, duração e conflitos da agenda.</div></div></PublicBookingShell>;
  if ((step === 4 || step === 5) && !dates.length) return <PublicBookingShell settings={settings}><div className={cardClass}><button type="button" onClick={() => resetFrom(modality === 'PRESENCIAL' ? 3 : 2)} className="mb-5 text-sm font-black text-violet-700"><ArrowLeft size={15} className="mr-1 inline" /> Voltar</button><StepHeader step={progressStep} title="Escolha o dia" text="Não há datas com horário reservável para esta combinação." /><div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center"><CalendarDays className="mx-auto text-slate-400" size={24} /><p className="mt-2 font-black">Nenhuma data disponível</p><p className="mt-1 text-sm text-slate-500">Escolha outra modalidade ou local, ou tente novamente mais tarde.</p></div></div></PublicBookingShell>;
  ...
  return <PublicBookingShell settings={settings}><div className="mb-4 flex items-center gap-1" aria-label={`Etapa ${progressStep} de ${totalSteps}`}>{Array.from({ length: totalSteps }, (_, index) => <span key={index} className={`h-1.5 flex-1 rounded-full ${index + 1 <= progressStep ? 'bg-violet-700' : 'bg-violet-100'}`} />)}</div>{step === 1 && <section className={cardClass}><StepHeader step={1} title={`Olá, sou ${settings.professionalName}`} text="Escolha um serviço publicado para começar seu agendamento." /><label className="block text-sm font-black text-slate-900">Escolha o Atendimento<select aria-label="Escolha o Atendimento" value={service?.id || ''} onChange={event => { const next = activeServices.find(item => item.id === event.target.value) || null; setService(next); setModality(null); setLocationId(undefined); setSelectedDate(''); setSelectedSlot(null); resetFrom(next ? 2 : 1); }} className={inputClass}><option value="">Selecione o atendimento</option>{activeServices.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></section>}
    ...
    {step === 4 && <section className={cardClass}><button type="button" onClick={() => resetFrom(modality === 'PRESENCIAL' ? 3 : 2)} className="mb-5 text-sm font-black text-violet-700"><ArrowLeft size={15} className="mr-1 inline" /> Voltar</button><StepHeader step={4} title="Escolha o dia" text={`Mostrando horários publicados para os próximos ${settings.maxAdvanceDays} dias.`} /><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{dates.slice(0, settings.maxAdvanceDays + 1).map(date => <button type="button" key={date} onClick={() => { setSelectedDate(date); setSelectedSlot(null); setError(''); setStep(6); }} className={`min-h-16 rounded-2xl border p-3 text-left transition ${selectedDate === date ? 'border-violet-600 bg-violet-50' : 'border-slate-200 bg-white hover:border-violet-300'}`}><span className="block text-xs font-black uppercase text-violet-700">{publicBookingWeekdayLabel(new Date(`${date}T12:00:00`).getDay())}</span><span className="mt-1 block text-sm font-black">{dateLabel(date)}</span></button>)}</div></section>}
    {step === 5 && <section className={cardClass}><button type="button" onClick={() => resetFrom(modality === 'PRESENCIAL' ? 3 : 2)} className="mb-5 text-sm font-black text-violet-700"><ArrowLeft size={15} className="mr-1 inline" /> Voltar</button><StepHeader step={progressStep} title="Escolha o dia" text="Selecione uma data para ver os horários livres." /><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{dates.slice(0, settings.maxAdvanceDays + 1).map(date => <button type="button" key={date} onClick={() => { setSelectedDate(date); setSelectedSlot(null); setError(''); setStep(6); }} className="min-h-16 rounded-2xl border border-slate-200 bg-white p-3 text-left transition hover:border-violet-300"><span className="block text-xs font-black uppercase text-violet-700">{publicBookingWeekdayLabel(new Date(`${date}T12:00:00`).getDay())}</span><span className="mt-1 block text-sm font-black">{dateLabel(date)}</span></button>)}</div></section>}
    {step === 6 && <section className={cardClass}><button type="button" onClick={() => resetFrom(5)} className="mb-5 text-sm font-black text-violet-700"><ArrowLeft size={15} className="mr-1 inline" /> Trocar dia</button><StepHeader step={progressStep} title="Escolha o horário" text={selectedDate ? `Horários em ${dateLabel(selectedDate)}.` : 'Escolha um dia para continuar.'} />{error && <p role="alert" className="mb-4 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-700">{error}</p>}{slots.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{slots.map(slot => <button type="button" key={`${slot.date}-${slot.time}`} onClick={() => { setSelectedSlot(slot); setError(''); resetFrom(7); }} className={`min-h-14 rounded-2xl border p-3 text-left transition ${selectedSlot?.time === slot.time ? 'border-violet-600 bg-violet-50' : 'border-slate-200 bg-white hover:border-violet-300'}`}><span className="block text-lg font-black">{slot.time}</span><span className="text-xs font-semibold text-slate-500">até {slot.endTime}</span></button>)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center"><CalendarDays className="mx-auto text-slate-400" size={24} /><p className="mt-2 font-black">Nenhum horário disponível neste dia</p><p className="mt-1 text-sm text-slate-500">Escolha outra data publicada.</p></div>} </section>}
