...
import type { PublicBookingAvailabilityPeriod, PublicBookingException, PublicBookingExceptionType, PublicBookingSettings } from './types';
...
export default function PublicBookingAvailabilityEditor({ settings, onSave }: { settings: PublicBookingSettings; onSave: (patch: Partial<PublicBookingSettings>, message: string) => Promise<void> }) {
  ...
  const routineSummary = habitualDays.filter(day => day.enabled && day.periods.length).map(day => `${dayLabels[day.dayOfWeek].slice(0, 3)} ${day.periods.map(period => `${period.startTime}–${period.endTime}`).join(' · ')}`).join(' · ') || 'Indisponível';
  ...
  const addPeriod = (dayOfWeek: number) => setHabitualDays(current => current.map(day => day.dayOfWeek === dayOfWeek ? { ...day, enabled: true, periods: [...day.periods, { dayOfWeek, enabled: true, startTime: '13:00', endTime: '17:00', modalities: settings.publishedModalities.filter(item => item.active).map(item => item.id), locationIds: settings.locations.filter(item => item.active).map(item => item.id) }] } : day));
  ...
  const saveException = async (type: PublicBookingExceptionType) => {
    ...
    const next: PublicBookingException = { id: `public-exception-${Date.now().toString(36)}`, professionalId: settings.professionalId, civilDate: selectedDate, type, startTime: type === 'BLOCK_DAY' ? undefined : exceptionStart, endTime: type === 'BLOCK_DAY' ? undefined : exceptionEnd, note: undefined, createdAt: now, updatedAt: now };
