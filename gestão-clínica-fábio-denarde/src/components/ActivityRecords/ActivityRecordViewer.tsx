...
export default function ActivityRecordViewer({ record, onClose }: { record: ActivityRecord | null; onClose: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    if (!record) return;
    setUrl(''); setError(''); setRetriedUrl(false);
  ...
  return (
    <Modal isOpen={!!record} onClose={onClose} title="Registro de atividade" width="max-w-4xl">
      {record && <div className="space-y-3">
        ...
        {url && <img src={url} alt={record.description || record.category} onError={handleMediaError} className="max-h-[70vh] w-full rounded-xl object-contain bg-black/5" />}
