...
import { CalendarDays, Camera, Filter, Image, Loader2, LockKeyhole, Plus, ShieldCheck } from 'lucide-react';
...
export default function ActivityRecordsTab({ patient, sessions, currentUserId, currentUserName, onOpenAuthorization }: Props) {
  ...
  }), [records, categoryFilter, sessionFilter, shareFilter, visibilityFilter, periodFilter]);

  const groupedRecords = useMemo(() => {
    const groups = new Map<string, ActivityRecord[]>();
    for (const record of filtered) {
      const current = groups.get(record.sessionDate) || [];
      current.push(record);
      groups.set(record.sessionDate, current);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast('Registro e foto excluídos com segurança.', 'success');
  ...
  return <div className="space-y-5 animate-in fade-in slide-in-from-top-2">
    <div className="flex flex-col gap-3 rounded-2xl border border-clinic-border bg-clinic-bg/60 p-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <div className="flex items-center gap-2"><Camera size={18} className="text-clinic-primary" /><h4 className="font-black text-clinic-text">Registros de atividades</h4></div>
        <p className="mt-1 text-xs text-clinic-text-muted">Fotos privadas associadas à criança e à sessão correta.</p>
      </div>
      <button type="button" onClick={handleNewRecord} className={`flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-black uppercase tracking-wide transition-all ${canRecord ? 'bg-clinic-primary text-white hover:bg-clinic-primary-hover' : 'border border-status-orange-text/30 bg-status-orange-bg text-status-orange-text hover:brightness-95'}`} title={canRecord ? 'Registrar uma nova atividade' : 'Ver motivo do bloqueio'}>{canRecord ? <Plus size={16} /> : <LockKeyhole size={16} />} Novo registro</button>
    </div>
    ...
    <div className="grid grid-cols-1 gap-2 rounded-xl border border-clinic-border bg-white p-3 sm:grid-cols-2 xl:grid-cols-5">
    ...
    {!loading && !error && filtered.length === 0 && <div className="rounded-2xl border border-dashed border-clinic-border bg-clinic-bg/40 p-10 text-center"><Image className="mx-auto mb-3 text-clinic-text-faint" /><p className="font-bold text-clinic-text">Nenhuma foto registrada</p><p className="text-xs text-clinic-text-muted">Os registros aparecerão aqui após o primeiro envio concluído.</p></div>}
    <div className="space-y-6">
      {groupedRecords.map(([date, dateRecords]) => (
        <section key={date} className="space-y-3">
          <div className="flex items-center gap-2 border-b border-clinic-border pb-2">
            <CalendarDays size={15} className="text-clinic-primary" />
            <h5 className="text-xs font-black uppercase tracking-wide text-clinic-text">{safeFormatDate(date, 'dd/MM/yyyy')}</h5>
            <span className="text-[10px] font-bold text-clinic-text-faint">{dateRecords.length} {dateRecords.length === 1 ? 'registro' : 'registros'}</span>
          </div>
    ...
    <Modal isOpen={!!deleteRecord} onClose={() => !busy && setDeleteRecord(null)} title="Excluir registro" width="max-w-md"><div className="space-y-5"><p className="text-sm text-clinic-text">A foto será removida do Google Drive e o registro será excluído. Esta ação não altera a sessão, presença ou pagamentos.</p><div className="flex justify-end gap-2"><button disabled={busy} onClick={() => setDeleteRecord(null)} className="rounded-lg bg-clinic-bg px-4 py-2 text-xs font-bold">Cancelar</button><button disabled={busy} onClick={() => void confirmDelete()} className="rounded-lg bg-status-red-text px-4 py-2 text-xs font-bold text-white">{busy ? 'Excluindo...' : 'Excluir'}</button></div></div></Modal>
  </div>;
}
