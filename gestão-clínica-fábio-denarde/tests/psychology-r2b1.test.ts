...
test('R2B1 UI 41 — backup funcional substitui a central técnica', () => { assert.match(importUi, /Backup dos meus dados/); assert.match(importUi, /Gerar backup/); });
test('R2B1 UI 42 — importação não aparece na UX profissional', () => assert.doesNotMatch(importUi, /Importar dados/));
test('R2B1 UI 43 — exportação técnica não aparece na UX profissional', () => assert.doesNotMatch(importUi, /Exportar \/ Fazer backup/));
test('R2B1 UI 44 — verificação técnica não aparece na UX profissional', () => assert.doesNotMatch(importUi, /Verificar backup/));
...
test('R2B1 UI 47 — simulação não aparece na UX profissional', () => assert.doesNotMatch(importUi, /Simular importação/));
...
test('R2B1 UI 50 — escopo autenticado é informado', () => { assert.match(importUi, /conta autenticada/); assert.match(importUi, /escopo autorizado/); });
