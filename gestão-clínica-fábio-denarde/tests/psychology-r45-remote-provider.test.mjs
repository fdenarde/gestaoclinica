...
test('R45 — Settings de leitura usa view e mutação permanece administrativa', () => {
  assert.match(
    psychologyApiSource,
    /resource === 'settings'[\s\S]*?req\.method === 'GET'[\s\S]*?requiredPermissions: \['settings\.clinic\.view'\]/,
  );
  assert.match(
    psychologyApiSource,
...
test('R45 — os três agregados autenticados retornam 200 com escopo profissional', async () => {
  ...
  const handler = createPsychologyApiHandler({
    ...
    resolveAccess: async (_req, options) => {
      requiredPermissions.push(options.requiredPermissions[0]);
      return runtimeScope;
    },
    auditLogger: () => {},
  });
  ...
  assert.deepEqual(requiredPermissions, ['patients.list', 'agenda.own.view', 'settings.clinic.view']);
});
...
test('R45 — authenticated remote carrega patients, sessions e settings separadamente com dados não vazios', async () => {
  ...
  assert.deepEqual(
    ...
    ['/api/psychology/patients', '/api/psychology/sessions', '/api/psychology/settings'],
  );
...
test('R45 — UI diferencia carregamento/erro de zero real e bloqueia fallback local durante falha', () => {
  ...
  assert.match(remoteClientSource, /const \[patients, sessions, settings\] = await Promise\.all/);
});
