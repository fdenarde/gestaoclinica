...
async function searchAny0900() {
    console.log("Searching for ANY session at 09:00...");
    const sessionsSnapshot = await db.collectionGroup('sessions')
        .get();
    
    for (const doc of sessionsSnapshot.docs) {
        const s = doc.data();
        if (s.time && (s.time.includes('09:00') || s.time.includes('09h'))) {
...
searchAny0900().then(() => process.exit(0));
